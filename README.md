...
Release 构建、i5-14600KF、2026-09-20 实测（`ParserBenchmark` 一次运行的中位数；同一台机器
上跑多次会有 ±5%~20% 抖动，长输入一列尤其明显）。

数量解析与生成式解析器（flex/bison 生成代码）的对照：两侧共用同一套 `Quantity` 运算、
同一份语料，且每次调用都逐位校验两侧结果一致。
...
读法：`sqrt(16) + abs(-7)` 是两个内置函数调用，折到单次约 0.5 µs，与宿主注册的
`taxed(100 mm)`（单次调用含一次数量乘法）同量级，看不出注册表这层额外的分派开销；
字典里的名字查找（虚调用 + 有序表）相对建树与求值可以忽略。序列两条最贵，因为除 AST
之外还要为序列本身与分量存储分配。

词法与解析的分工：319 字节的长链，只跑词法 12.1 µs（26.5 MB/s），走完整解析 37.0 µs，
50 层括号 5.2 µs。
