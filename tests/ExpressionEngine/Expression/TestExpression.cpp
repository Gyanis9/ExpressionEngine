...
ExpressionPtr binary(Operator operation, ExpressionPtr left, ExpressionPtr right) {
    return std::make_unique<OperatorExpression>(
        nullptr, std::move(left), operation, std::move(right));
}
...
ExpressionPtr function(const Function functionKind, Arguments&&... arguments) {
    ...
    return std::make_unique<FunctionExpression>(
        nullptr, functionKind, name, std::move(parameterList));
}
...
public:
     ...
     */
    FakeProperty(std::string propertyName,
                 std::string propertyTypeName,
                 std::optional<Value> initialValue)
        : m_name(std::move(propertyName)), m_typeName(std::move(propertyTypeName)),
...
public:
    ...
    void addProperty(std::string propertyName,
        ...
        m_properties.emplace_back(
            std::move(propertyName), std::move(propertyTypeName), std::move(initialValue));
    }
...
public:
    /// 覆写：按文档名与对象名解析对象
    [[nodiscard]] IObject* resolve(std::string_view documentName,
                                   std::string_view objectName) override {
        if (objectName.empty()) {
...
TEST(ExpressionTest, NodeTextKeepsPrecedence) {
    ...
    auto nestedSubtract = binary(
        Operator::Subtract, number(10.0), binary(Operator::Subtract, number(3.0), number(1.0)));
    EXPECT_EQ(nestedSubtract->toString(), "10 - (3 - 1)");
    // 同优先级但运算符不同，右侧同样补括号
    auto mixedPriority =
        binary(Operator::Subtract, number(10.0), binary(Operator::Add, number(3.0), number(1.0)));
    EXPECT_EQ(mixedPriority->toString(), "10 - (3 + 1)");

    // 一元运算符不写右操作数
    auto negate =
        std::make_unique<OperatorExpression>(nullptr, number(5.0), Operator::Negate, nullptr);
...
TEST(ExpressionTest, ArithmeticEvaluation) {
    EXPECT_DOUBLE_EQ(
        quantityOf(binary(Operator::Add, number(2.0), number(3.0))->evaluate()).getValue(), 5.0);
    EXPECT_DOUBLE_EQ(
        quantityOf(binary(Operator::Subtract, number(2.0), number(3.0))->evaluate()).getValue(),
        -1.0);
    EXPECT_DOUBLE_EQ(
        quantityOf(binary(Operator::Multiply, number(2.0), number(3.0))->evaluate()).getValue(),
        6.0);
    EXPECT_DOUBLE_EQ(
        quantityOf(binary(Operator::Divide, number(7.0), number(2.0))->evaluate()).getValue(), 3.5);
    EXPECT_DOUBLE_EQ(
        quantityOf(binary(Operator::Modulo, number(7.0), number(2.0))->evaluate()).getValue(), 1.0);
    EXPECT_DOUBLE_EQ(
        quantityOf(binary(Operator::Power, number(2.0), number(10.0))->evaluate()).getValue(),
        1024.0);

    // 树形决定结合顺序：(2 + 3) * 4 = 20
    auto sum = binary(Operator::Add, number(2.0), number(3.0));
    EXPECT_DOUBLE_EQ(
        quantityOf(binary(Operator::Multiply, std::move(sum), number(4.0))->evaluate()).getValue(),
        20.0);
    ...
    // 除数为零：报错而不是给出 inf
    EXPECT_THROW(static_cast<void>(binary(Operator::Divide, number(1.0), number(0.0))->evaluate()),
                 Base::ValueError);
    EXPECT_THROW(static_cast<void>(binary(Operator::Modulo, number(1.0), number(0.0))->evaluate()),
                 Base::ValueError);

    // 类型不符：几何值不能参与加法
    EXPECT_THROW(static_cast<void>(
                     binary(Operator::Add, number(1.0), vectorNode(1.0, 2.0, 3.0))->evaluate()),
                 Base::TypeError);

    // 文本相加是拼接
    auto textSum = binary(Operator::Add,
                          std::make_unique<StringExpression>(nullptr, "零件 "),
                          std::make_unique<StringExpression>(nullptr, "A"));
    EXPECT_EQ(textOf(textSum->evaluate()), "零件 A");
    // 文本与数量相加报错
    EXPECT_THROW(
        static_cast<void>(
            binary(Operator::Add, std::make_unique<StringExpression>(nullptr, "零件"), number(1.0))
                ->evaluate()),
        Base::TypeError);

    // 构造期拒绝缺操作数与一元运算带右操作数
    EXPECT_THROW(static_cast<void>(std::make_unique<OperatorExpression>(
                     nullptr, nullptr, Operator::Add, number(1.0))),
                 EvaluationError);
    EXPECT_THROW(static_cast<void>(std::make_unique<OperatorExpression>(
                     nullptr, number(1.0), Operator::Add, nullptr)),
                 EvaluationError);
    EXPECT_THROW(static_cast<void>(std::make_unique<OperatorExpression>(
                     nullptr, number(1.0), Operator::Negate, number(2.0))),
                 EvaluationError);
}
...
TEST(ExpressionTest, UnitPropagation) {
    // 2 mm：数值节点与单位节点相乘
    auto twoMillimetres = std::make_unique<OperatorExpression>(
        nullptr,
        number(2.0),
        Operator::UnitScale,
    ...
    // 2 mm * 3 mm = 6 mm^2
    const Units::Quantity area = quantityOf(binary(Operator::Multiply,
                                                   quantity(2.0, Units::Unit::Length),
                                                   quantity(3.0, Units::Unit::Length))
                                                ->evaluate());
    EXPECT_EQ(area.getUnit(), Units::Unit::Area);
    EXPECT_DOUBLE_EQ(area.getValue(), 6.0);

    // 面积除以长度得到长度
    const Units::Quantity quotient = quantityOf(binary(Operator::Divide,
                                                       quantity(6.0, Units::Unit::Area),
                                                       quantity(2.0, Units::Unit::Length))
                                                    ->evaluate());
    ...
    const Units::Quantity volume = quantityOf(
        binary(Operator::Power, quantity(2.0, Units::Unit::Length), number(3.0))->evaluate());
    EXPECT_EQ(volume.getUnit(), Units::Unit::Volume);
    EXPECT_DOUBLE_EQ(volume.getValue(), 8.0);

    // 分数指数只要让各量纲指数落在整数格点上就合法：面积开平方是长度
    const Units::Quantity side = quantityOf(
        binary(Operator::Power, quantity(4.0, Units::Unit::Area), number(0.5))->evaluate());
    ...
    EXPECT_THROW(
        static_cast<void>(
            binary(Operator::Power, quantity(4.0, Units::Unit::Length), number(0.5))->evaluate()),
        Base::UnitsMismatchError);
    // 指数带量纲报错
    EXPECT_THROW(static_cast<void>(binary(Operator::Power,
                                          quantity(2.0, Units::Unit::Length),
                                          quantity(2.0, Units::Unit::Length))
                                       ->evaluate()),
                 Base::UnitsMismatchError);
    ...
    // 加减要求同量纲
    EXPECT_THROW(static_cast<void>(binary(Operator::Add,
                                          quantity(1.0, Units::Unit::Length),
                                          quantity(1.0, Units::Unit::TimeSpan))
                                       ->evaluate()),
                 Base::UnitsMismatchError);
    // 长度量与纯数不能相加
    EXPECT_THROW(
        static_cast<void>(
            binary(Operator::Add, quantity(1.0, Units::Unit::Length), number(1.0))->evaluate()),
        Base::UnitsMismatchError);

    // 取余：同量纲可算，异量纲报错
    EXPECT_DOUBLE_EQ(quantityOf(binary(Operator::Modulo,
                                       quantity(7.0, Units::Unit::Length),
                                       quantity(2.0, Units::Unit::Length))
                                    ->evaluate())
                         .getValue(),
                     1.0);
    EXPECT_THROW(static_cast<void>(binary(Operator::Modulo,
                                          quantity(7.0, Units::Unit::Length),
                                          quantity(2.0, Units::Unit::TimeSpan))
                                       ->evaluate()),
                 Base::UnitsMismatchError);
}
...
TEST(ExpressionTest, ComparisonSemantics) {
    ...
    EXPECT_TRUE(boolOf(binary(Operator::Equal,
                              quantity(1.0, Units::Unit::Length),
                              quantity(1.0, Units::Unit::Length))
                           ->evaluate()));
    // 量纲不同时相等判定给 false，供 == 复用
    EXPECT_FALSE(boolOf(binary(Operator::Equal,
                               quantity(1.0, Units::Unit::Length),
                               quantity(1.0, Units::Unit::TimeSpan))
                            ->evaluate()));
    EXPECT_TRUE(boolOf(binary(Operator::NotEqual,
                              quantity(1.0, Units::Unit::Length),
                              quantity(1.0, Units::Unit::TimeSpan))
                           ->evaluate()));
    // 但大小比较量纲不同必须报错
    EXPECT_THROW(static_cast<void>(binary(Operator::Less,
                                          quantity(1.0, Units::Unit::Length),
                                          quantity(1.0, Units::Unit::TimeSpan))
                                       ->evaluate()),
                 Base::UnitsMismatchError);
    ...
    // 几何值按容差相等，不参与大小比较
    EXPECT_TRUE(boolOf(
        binary(Operator::Equal, vectorNode(1.0, 0.0, 0.0), vectorNode(1.0, 0.0, 0.0))->evaluate()));
    EXPECT_FALSE(boolOf(
        binary(Operator::Equal, vectorNode(1.0, 0.0, 0.0), vectorNode(2.0, 0.0, 0.0))->evaluate()));
    EXPECT_THROW(static_cast<void>(
                     binary(Operator::Less, vectorNode(1.0, 0.0, 0.0), vectorNode(2.0, 0.0, 0.0))
                         ->evaluate()),
                 Base::TypeError);
}
...
TEST(ExpressionTest, ConditionalSemantics) {
    auto conditional =
        std::make_unique<ConditionalExpression>(nullptr,
                                                binary(Operator::Less, number(1.0), number(2.0)),
                                                std::make_unique<StringExpression>(nullptr, "yes"),
    ...
    auto falseBranch =
        std::make_unique<ConditionalExpression>(nullptr, number(0.0), number(1.0), number(2.0));
    EXPECT_DOUBLE_EQ(quantityOf(falseBranch->evaluate()).getValue(), 2.0);

    // 分支优先级不高于条件运算符时补括号
    auto nested = std::make_unique<ConditionalExpression>(
        nullptr, number(1.0), binary(Operator::Less, number(2.0), number(3.0)), number(4.0));
    EXPECT_EQ(nested->toString(), "1 ? (2 < 3) : 4");

    // 条件不能判定真假时报错，而不是沿用 Python 的真值规则
    auto textCondition = std::make_unique<ConditionalExpression>(
        nullptr, std::make_unique<StringExpression>(nullptr, "x"), number(1.0), number(2.0));
    ...
    // 构造期拒绝缺分支
    EXPECT_THROW(static_cast<void>(std::make_unique<ConditionalExpression>(
                     nullptr, number(1.0), number(2.0), nullptr)),
                 EvaluationError);
}
...
TEST(ExpressionTest, ScalarFunctions) {
    // 三角函数按角度输入（库内角度量以度存储）
    EXPECT_NEAR(
        quantityOf(function(Function::Sine, number(30.0))->evaluate()).getValue(), 0.5, 1e-12);
    EXPECT_NEAR(
        quantityOf(function(Function::Cosine, quantity(60.0, Units::Unit::Angle))->evaluate())
            .getValue(),
        0.5,
        1e-12);
    // 反三角函数输出角度量：asin(1) = 90 度
    const Units::Quantity arcsine =
        quantityOf(function(Function::ArcSine, number(1.0))->evaluate());
    EXPECT_EQ(arcsine.getUnit(), Units::Unit::Angle);
    EXPECT_NEAR(arcsine.getValue(), 90.0, 1e-9);

    // 取整与绝对值保持量纲
    const Units::Quantity rounded =
        quantityOf(function(Function::Round, quantity(2.6, Units::Unit::Length))->evaluate());
    EXPECT_EQ(rounded.getUnit(), Units::Unit::Length);
    EXPECT_DOUBLE_EQ(rounded.getValue(), 3.0);
    EXPECT_DOUBLE_EQ(
        quantityOf(function(Function::Truncate, quantity(2.6, Units::Unit::Length))->evaluate())
            .getValue(),
        2.0);
    EXPECT_DOUBLE_EQ(
        quantityOf(function(Function::Floor, quantity(-2.5, Units::Unit::Length))->evaluate())
            .getValue(),
        -3.0);
    EXPECT_DOUBLE_EQ(
        quantityOf(function(Function::Ceiling, quantity(2.1, Units::Unit::Length))->evaluate())
            .getValue(),
        3.0);
    EXPECT_DOUBLE_EQ(
        quantityOf(function(Function::Absolute, quantity(-3.0, Units::Unit::Length))->evaluate())
            .getValue(),
        3.0);

    // 平方根取量纲的平方根：sqrt(4 mm^2) = 2 mm
    const Units::Quantity root =
        quantityOf(function(Function::SquareRoot, quantity(4.0, Units::Unit::Area))->evaluate());
    EXPECT_EQ(root.getUnit(), Units::Unit::Length);
    EXPECT_DOUBLE_EQ(root.getValue(), 2.0);
    // 立方根同理
    EXPECT_EQ(
        quantityOf(function(Function::CubeRoot, quantity(27.0, Units::Unit::Volume))->evaluate())
            .getUnit(),
        Units::Unit::Length);

    EXPECT_DOUBLE_EQ(
        quantityOf(function(Function::Power, number(2.0), number(10.0))->evaluate()).getValue(),
        1024.0);
    EXPECT_DOUBLE_EQ(
        quantityOf(function(Function::Modulo, number(7.0), number(2.0))->evaluate()).getValue(),
        1.0);
    EXPECT_DOUBLE_EQ(
        quantityOf(function(Function::Hypotenuse, number(3.0), number(4.0))->evaluate()).getValue(),
        5.0);
    EXPECT_DOUBLE_EQ(
        quantityOf(function(Function::Cathetus, number(5.0), number(4.0))->evaluate()).getValue(),
        3.0);
    EXPECT_NEAR(
        quantityOf(function(Function::Logarithm, number(std::numbers::e))->evaluate()).getValue(),
        1.0,
        1e-12);
    // 对数的参数必须是纯数
    EXPECT_THROW(
        static_cast<void>(
            function(Function::Logarithm, quantity(10.0, Units::Unit::Length))->evaluate()),
        Base::UnitsMismatchError);

    const Units::Quantity angle =
        quantityOf(function(Function::ArcTangent2, number(1.0), number(1.0))->evaluate());
    EXPECT_EQ(angle.getUnit(), Units::Unit::Angle);
    EXPECT_NEAR(angle.getValue(), 45.0, 1e-9);
    // atan2 要求两侧单位一致
    EXPECT_THROW(static_cast<void>(function(Function::ArcTangent2,
                                            quantity(1.0, Units::Unit::Length),
                                            quantity(1.0, Units::Unit::TimeSpan))
                                       ->evaluate()),
                 Base::UnitsMismatchError);
    ...
    // 逻辑非只看数值
    EXPECT_DOUBLE_EQ(quantityOf(function(Function::LogicalNot, number(0.0))->evaluate()).getValue(),
                     1.0);
    EXPECT_DOUBLE_EQ(quantityOf(function(Function::LogicalNot, number(2.0))->evaluate()).getValue(),
                     0.0);

    // 文本函数：str 排版取值，parsequant 解析数量文本
    EXPECT_EQ(textOf(function(Function::Stringify, quantity(2.0, Units::Unit::Length))->evaluate())
                  .find("2"),
              std::size_t(0));
    const Units::Quantity parsed = quantityOf(
        function(Function::ParseQuantity, std::make_unique<StringExpression>(nullptr, "1.5 mm"))
            ->evaluate());
    EXPECT_DOUBLE_EQ(parsed.getValue(), 1.5);
    EXPECT_EQ(parsed.getUnit(), Units::Unit::Length);
    EXPECT_THROW(static_cast<void>(function(Function::ParseQuantity,
                                            std::make_unique<StringExpression>(nullptr, "abc"))
                                       ->evaluate()),
                 Base::ParserError);
    ...
    EXPECT_THROW(
        static_cast<void>(
            function(Function::Address, number(1.0), number(1.0), number(9.0))->evaluate()),
        Base::ValueError);

    // 参数个数错：构造期即报错
    EXPECT_THROW(static_cast<void>(function(Function::SquareRoot, number(4.0), number(4.0))),
                 EvaluationError);
    ...
    // 哨兵值不是函数
    EXPECT_THROW(static_cast<void>(std::make_unique<FunctionExpression>(
                     nullptr, Function::None, std::string(), std::vector<ExpressionPtr>())),
                 Base::ParserError);
}
...
TEST(ExpressionTest, VectorFunctions) {
    EXPECT_TRUE(
        vectorOf(function(Function::Vector, number(1.0), number(2.0), number(3.0))->evaluate())
            .IsEqual(Base::Vector3d(1.0, 2.0, 3.0), 1e-12));

    const Base::Vector3d normalized =
        vectorOf(function(Function::VectorNormalize, vectorNode(3.0, 0.0, 0.0))->evaluate());
    EXPECT_NEAR(normalized.x, 1.0, 1e-12);
    EXPECT_NEAR(normalized.y, 0.0, 1e-12);
    // 零向量没有方向
    EXPECT_THROW(static_cast<void>(
                     function(Function::VectorNormalize, vectorNode(0.0, 0.0, 0.0))->evaluate()),
                 Base::ValueError);

    // 点积是无量纲纯数
    EXPECT_DOUBLE_EQ(
        doubleOf(function(Function::VectorDot, vectorNode(1.0, 0.0, 0.0), vectorNode(2.0, 3.0, 4.0))
                     ->evaluate()),
        2.0);
    // 叉积
    const Base::Vector3d cross = vectorOf(
        function(Function::VectorCross, vectorNode(1.0, 0.0, 0.0), vectorNode(0.0, 1.0, 0.0))
            ->evaluate());
    EXPECT_NEAR(cross.z, 1.0, 1e-12);
    // 夹角带角度单位
    const Units::Quantity angle = quantityOf(
        function(Function::VectorAngle, vectorNode(1.0, 0.0, 0.0), vectorNode(0.0, 1.0, 0.0))
            ->evaluate());
    EXPECT_EQ(angle.getUnit(), Units::Unit::Angle);
    EXPECT_NEAR(angle.getValue(), 90.0, 1e-9);

    // 缩放：分量要求长度量或纯数
    const Base::Vector3d scaled = vectorOf(
        function(
            Function::VectorScale, vectorNode(1.0, 1.0, 1.0), number(2.0), number(3.0), number(4.0))
            ->evaluate());
    ...
    EXPECT_NEAR(
        vectorOf(
            function(Function::VectorScaleY, vectorNode(1.0, 1.0, 1.0), number(5.0))->evaluate())
            .y,
        5.0,
        1e-12);
    EXPECT_THROW(static_cast<void>(function(Function::VectorScaleX,
                                            vectorNode(1.0, 1.0, 1.0),
                                            quantity(5.0, Units::Unit::TimeSpan))
                                       ->evaluate()),
                 Base::UnitsMismatchError);

    // 点到直线的距离带长度单位
    const Units::Quantity distance = quantityOf(function(Function::VectorLineDistance,
                                                         vectorNode(0.0, 1.0, 0.0),
                                                         vectorNode(0.0, 0.0, 0.0),
                                                         vectorNode(1.0, 0.0, 0.0))
                                                    ->evaluate());
    EXPECT_EQ(distance.getUnit(), Units::Unit::Length);
    EXPECT_NEAR(distance.getValue(), 1.0, 1e-12);
    // 点在线上的投影
    const Base::Vector3d projection = vectorOf(function(Function::VectorLineProjection,
                                                        vectorNode(1.0, 2.0, 0.0),
                                                        vectorNode(0.0, 0.0, 0.0),
                                                        vectorNode(1.0, 0.0, 0.0))
                                                   ->evaluate());
    ...
    EXPECT_THROW(
        static_cast<void>(
            function(Function::VectorDot, number(1.0), vectorNode(1.0, 0.0, 0.0))->evaluate()),
        Base::TypeError);
}
...
TEST(ExpressionTest, MatrixAndPlacementFunctions) {
    // 16 个分量按行优先拼出单位矩阵
    EXPECT_TRUE(matrixOf(function(Function::Matrix,
                                  number(1.0),
                                  number(0.0),
                                  number(0.0),
                                  number(0.0),
    ...
    EXPECT_THROW(static_cast<void>(
                     function(Function::Matrix, number(1.0), number(0.0), number(0.0))->evaluate()),
                 EvaluationError);

    // translationm 造平移矩阵
    const Base::Matrix4D translation =
        matrixOf(function(Function::TranslationMatrix, vectorNode(1.0, 2.0, 3.0))->evaluate());
    EXPECT_NEAR(translation[0][3], 1.0, 1e-12);
    EXPECT_NEAR(translation[1][3], 2.0, 1e-12);
    EXPECT_NEAR(translation[2][3], 3.0, 1e-12);
    // 三个分量形式要求长度量或纯数
    EXPECT_THROW(static_cast<void>(function(Function::TranslationMatrix,
                                            quantity(1.0, Units::Unit::TimeSpan),
                                            number(2.0),
                                            number(3.0))
                                       ->evaluate()),
    ...
    auto rotateZ =
        function(Function::MatrixRotateZ, valueNode(Value(Base::Placement())), number(90.0));
    ...
    // 旋转角度必须是无量纲或角度量
    EXPECT_THROW(static_cast<void>(function(Function::MatrixRotateZ,
                                            valueNode(Value(Base::Placement())),
                                            quantity(1.0, Units::Unit::Length))
                                       ->evaluate()),
                 Base::UnitsMismatchError);
    // 第一个参数必须是矩阵、位姿或旋转
    EXPECT_THROW(
        static_cast<void>(function(Function::MatrixRotateZ, number(1.0), number(90.0))->evaluate()),
        Base::TypeError);

    // mscale：两个参数时第二个参数是向量
    const Base::Matrix4D scaledMatrix =
        matrixOf(function(Function::MatrixScale,
                          valueNode(Value(Base::Matrix4D())),
                          function(Function::Vector, number(2.0), number(2.0), number(2.0)))
                     ->evaluate());
    const Base::Vector3d scaledPoint = scaledMatrix * Base::Vector3d(1.0, 1.0, 1.0);
    EXPECT_NEAR(scaledPoint.x, 2.0, 1e-12);

    // minvert：位姿求逆得到反向平移
    const Base::Placement inverted = placementOf(
        function(Function::MatrixInvert,
                 valueNode(Value(Base::Placement(Base::Vector3d(1.0, 2.0, 3.0), Base::Rotation()))))
            ->evaluate());
    ...
    singular.scale(0.0);
    EXPECT_THROW(
        static_cast<void>(function(Function::MatrixInvert, valueNode(Value(singular)))->evaluate()),
        Base::ValueError);
    EXPECT_THROW(static_cast<void>(function(Function::MatrixInvert, number(1.0))->evaluate()),
                 Base::TypeError);

    // rotation：轴加角度（按度）
    const Base::Rotation rotation = rotationOf(
        function(Function::Rotation, vectorNode(0.0, 0.0, 1.0), number(90.0))->evaluate());
    ...
    EXPECT_TRUE(
        rotationOf(function(Function::Rotation, number(0.0), number(0.0), number(0.0))->evaluate())
            .isIdentity());
    // 第一个参数必须是轴向量
    EXPECT_THROW(
        static_cast<void>(function(Function::Rotation, number(1.0), number(2.0))->evaluate()),
        Base::TypeError);

    // placement(位置; 旋转)
    const Base::Placement placement =
        placementOf(function(Function::Placement,
                             vectorNode(1.0, 2.0, 3.0),
                             function(Function::Rotation, vectorNode(0.0, 0.0, 1.0), number(0.0)))
                        ->evaluate());
    EXPECT_NEAR(placement.getPosition().y, 2.0, 1e-12);
    // 单参数形式需要矩阵或位姿
    EXPECT_THROW(static_cast<void>(function(Function::Placement, number(1.0))->evaluate()),
                 Base::TypeError);
}
...
TEST(ExpressionTest, AggregateFunctions) {
    EXPECT_DOUBLE_EQ(
        quantityOf(function(Function::Sum, number(1.0), number(2.0), number(3.0))->evaluate())
            .getValue(),
        6.0);
    EXPECT_DOUBLE_EQ(
        quantityOf(function(Function::Average, number(1.0), number(2.0), number(3.0))->evaluate())
            .getValue(),
        2.0);
    EXPECT_DOUBLE_EQ(
        quantityOf(function(Function::Minimum, number(3.0), number(1.0))->evaluate()).getValue(),
        1.0);
    EXPECT_DOUBLE_EQ(
        quantityOf(function(Function::Maximum, number(3.0), number(1.0))->evaluate()).getValue(),
        3.0);
    EXPECT_DOUBLE_EQ(
        quantityOf(function(Function::Count, number(3.0), number(1.0))->evaluate()).getValue(),
        2.0);

    // 逻辑聚合只看数值
    EXPECT_DOUBLE_EQ(
        quantityOf(function(Function::LogicalAnd, number(1.0), number(0.0))->evaluate()).getValue(),
        0.0);
    EXPECT_DOUBLE_EQ(
        quantityOf(function(Function::LogicalOr, number(0.0), number(2.0))->evaluate()).getValue(),
        1.0);

    // 求和保持量纲，混量纲报错
    const Units::Quantity total = quantityOf(function(Function::Sum,
                                                      quantity(1.0, Units::Unit::Length),
                                                      quantity(2.0, Units::Unit::Length))
                                                 ->evaluate());
    EXPECT_EQ(total.getUnit(), Units::Unit::Length);
    EXPECT_DOUBLE_EQ(total.getValue(), 3.0);
    EXPECT_THROW(static_cast<void>(function(Function::Sum,
                                            quantity(1.0, Units::Unit::Length),
                                            quantity(2.0, Units::Unit::TimeSpan))
                                       ->evaluate()),
                 Base::UnitsMismatchError);
    ...
    // 样本标准差：长度量的标准差仍是长度
    const Units::Quantity deviation = quantityOf(function(Function::StandardDeviation,
                                                          quantity(1.0, Units::Unit::Length),
                                                          quantity(3.0, Units::Unit::Length))
                                                     ->evaluate());
    ...
    // 单样本上样本标准差没有定义
    EXPECT_THROW(static_cast<void>(function(Function::StandardDeviation, number(1.0))->evaluate()),
                 EvaluationError);

    // 文本参与聚合是类型错，不静默跳过
    EXPECT_THROW(static_cast<void>(
                     function(Function::Sum, std::make_unique<StringExpression>(nullptr, "abc"))
                         ->evaluate()),
                 Base::TypeError);
}
...
TEST(ExpressionTest, RangeAggregateReadsCells) {
    ...
    auto sum =
        std::make_unique<FunctionExpression>(&resolver, Function::Sum, "sum", std::move(arguments));
    ...
    auto partialSum = std::make_unique<FunctionExpression>(
        &resolver, Function::Sum, "sum", std::move(partialArguments));
    ...
    auto unboundSum = std::make_unique<FunctionExpression>(
        nullptr, Function::Sum, "sum", std::move(unboundArguments));
...
TEST(ExpressionTest, VariableResolutionAndWriteBack) {
    ...
    EXPECT_THROW(variable->assignValue(Value(Units::Quantity(1.0, Units::Unit::Length))),
                 Base::AttributeError);
...
TEST(ExpressionTest, ComponentsAreStoredButRejected) {
    ...
    variable->addComponent(Expression::Component::arrayIndex(
        std::make_unique<NumberExpression>(nullptr, Units::Quantity(0.0))));
    ...
    ranged->addComponent(Expression::Component::rangeComponent(
        std::make_unique<NumberExpression>(nullptr, Units::Quantity(1.0)),
...
TEST(ExpressionTest, SimplifyFoldsConstantsOnly) {
    ...
    auto variable = std::make_unique<VariableExpression>(
        ...
        VariableExpression::Reference{
            .documentName = "", .objectName = "Box", .propertyName = "Length"});
...
TEST(ExpressionTest, ValueExpressionWrapsEvaluatedValues) {
    ...
    EXPECT_EQ(
        makeValueExpression(nullptr, Value(Units::Quantity(2.0, Units::Unit::Length)))->nodeName(),
        "Number");
