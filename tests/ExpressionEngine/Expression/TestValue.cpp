...
TEST(ValueTest, QuantityConversionAcceptsNumbersAndBoolean) {
    EXPECT_DOUBLE_EQ(
        quantityOf(toQuantity(Value(Units::Quantity(2.0, Units::Unit::Length)), "测试")).getValue(),
        2.0);
    EXPECT_EQ(
        quantityOf(toQuantity(Value(Units::Quantity(2.0, Units::Unit::Length)), "测试")).getUnit(),
        Units::Unit::Length);
...
TEST(ValueTest, QuantityConversionParsesText) {
    ...
    EXPECT_THROW(static_cast<void>(toQuantity(Value(std::string("abc")), "属性 Length")),
                 Base::ParserError);
    EXPECT_THROW(static_cast<void>(toQuantity(Value(std::string("12 个")), "属性 Length")),
                 Base::ParserError);
}
...
TEST(ValueTest, QuantityConversionRejectsGeometry) {
    EXPECT_THROW(
        static_cast<void>(toQuantity(Value(Base::Vector3d(1.0, 2.0, 3.0)), "加法左操作数")),
        Base::TypeError);
    EXPECT_THROW(static_cast<void>(toQuantity(Value(Base::Matrix4D()), "加法左操作数")),
                 Base::TypeError);
    EXPECT_THROW(static_cast<void>(toQuantity(Value(Base::Rotation()), "加法左操作数")),
                 Base::TypeError);
}
...
TEST(ValueTest, DoubleConversionRejectsDimensionedAndText) {
    ...
    EXPECT_THROW(
        static_cast<void>(toDouble(Value(Units::Quantity(3.0, Units::Unit::Length)), "测试")),
        Base::TypeError);
    EXPECT_THROW(static_cast<void>(toDouble(Value(std::string("3")), "测试")), Base::TypeError);
    EXPECT_THROW(static_cast<void>(toDouble(Value(Base::Vector3d(1.0, 2.0, 3.0)), "测试")),
                 Base::TypeError);
}
...
TEST(ValueTest, BooleanConversion) {
    ...
    EXPECT_THROW(static_cast<void>(toBool(Value(std::string("true")), "条件表达式")),
                 Base::TypeError);
    EXPECT_THROW(static_cast<void>(toBool(Value(Base::Vector3d(1.0, 2.0, 3.0)), "条件表达式")),
                 Base::TypeError);
}
...
TEST(ValueTest, EqualityRequiresSameType) {
    EXPECT_TRUE(valuesEqual(Value(Units::Quantity(1.0, Units::Unit::Length)),
                            Value(Units::Quantity(1.0, Units::Unit::Length))));
    // 量纲不同时返回 false 而不是抛错，供 == 运算符复用
    EXPECT_FALSE(valuesEqual(Value(Units::Quantity(1.0, Units::Unit::Length)),
                             Value(Units::Quantity(1.0, Units::Unit::TimeSpan))));
    // 类型不同一律不相等，不做跨类型换算
    ...
    EXPECT_TRUE(
        valuesEqual(Value(Base::Vector3d(0.0, 0.0, 0.0)), Value(Base::Vector3d(1e-9, 0.0, 0.0))));
    EXPECT_FALSE(
        valuesEqual(Value(Base::Vector3d(0.0, 0.0, 0.0)), Value(Base::Vector3d(1e-3, 0.0, 0.0))));
    EXPECT_TRUE(valuesEqual(Value(Base::Rotation()),
                            Value(Base::Rotation(Base::Vector3d(0.0, 0.0, 1.0), 0.0))));
}
...
TEST(ValueTest, LessThanRequiresComparableValues) {
    EXPECT_TRUE(valueLessThan(Value(Units::Quantity(1.0, Units::Unit::Length)),
                              Value(Units::Quantity(2.0, Units::Unit::Length))));
    EXPECT_FALSE(valueLessThan(Value(Units::Quantity(2.0, Units::Unit::Length)),
                               Value(Units::Quantity(1.0, Units::Unit::Length))));
    ...
    EXPECT_THROW(
        static_cast<void>(valueLessThan(Value(Units::Quantity(1.0, Units::Unit::Length)),
                                        Value(Units::Quantity(1.0, Units::Unit::TimeSpan)))),
        Base::UnitsMismatchError);
    EXPECT_THROW(static_cast<void>(
                     valueLessThan(Value(Units::Quantity(1.0, Units::Unit::Length)), Value(1.0))),
                 Base::UnitsMismatchError);
    // 几何值之间没有大小关系
    EXPECT_THROW(static_cast<void>(valueLessThan(Value(Base::Vector3d(1.0, 0.0, 0.0)),
                                                 Value(Base::Vector3d(2.0, 0.0, 0.0)))),
                 Base::TypeError);
    EXPECT_THROW(static_cast<void>(valueLessThan(Value(std::string("a")), Value(1.0))),
                 Base::TypeError);
}
