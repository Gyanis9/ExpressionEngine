...
void expectScanResult(const std::string_view input,
                      const NumericLocaleContext& locale,
                      const std::string_view canonicalText,
                      const double value,
...
void expectDiagnostic(const std::string_view input,
                      const NumericLocaleContext& locale,
                      const LocalizedNumberResult::Status status,
                      const NumericDiagnosticKind kind,
                      const NumericSyntaxContext syntax = NumericSyntaxContext::Standalone) {
    const auto result = scanLocalizedNumber(input, locale, syntax);
...
void expectCanonicalRoundTrip(const std::string_view input, const NumericLocaleContext& locale) {
    ...
    const auto conversion =
        std::from_chars(result.canonicalText.data(),
                        result.canonicalText.data() + result.canonicalText.size(),
                        parsed,
                        std::chars_format::general);
    ASSERT_EQ(conversion.ec, std::errc{}) << result.canonicalText;
    ASSERT_EQ(conversion.ptr, result.canonicalText.data() + result.canonicalText.size())
...
TEST(NumericInputTest, GrammarPolicyKeepsLocaleSeparatorsOutsideFunctionArguments) {
    ...
    for (const NumericSyntaxContext syntax :
         {NumericSyntaxContext::Standalone, NumericSyntaxContext::Expression}) {
        const NumericGrammarPolicy policy =
            ExpressionEngine::Base::numericGrammarPolicy(german, syntax);
...
TEST(NumericInputTest, GrammarPolicyDisablesGroupingWhenSeparatorsCollide) {
    ...
    const NumericGrammarPolicy german = ExpressionEngine::Base::numericGrammarPolicy(
        expectContext("de_DE"), NumericSyntaxContext::FunctionArgument);
    EXPECT_EQ(german.argumentSeparator, ";");
    EXPECT_TRUE(german.allowGrouping);

    // 逗号仍是实参分隔符：英语区域的分组分隔符就是逗号，必须关闭分组以免误吞实参
    const NumericGrammarPolicy english = ExpressionEngine::Base::numericGrammarPolicy(
        expectContext("en_US"), NumericSyntaxContext::FunctionArgument);
...
TEST(NumericInputTest, RejectsWrongGroupingSizes) {
    ...
    // 末组只有两位
    expectDiagnostic("1.234.56",
                     german,
                     LocalizedNumberResult::Status::Invalid,
    ...
    expectDiagnostic(",5",
                     cLocaleContext(),
                     LocalizedNumberResult::Status::Invalid,
                     NumericDiagnosticKind::ExpectedDigit);
    // 英语区域里 "1,5" 的分组位数不足三位
    expectDiagnostic("1,5",
                     english,
                     LocalizedNumberResult::Status::Invalid,
                     NumericDiagnosticKind::InvalidGrouping);
}
...
TEST(NumericInputTest, ReportsIncompleteTokens) {
    ...
    expectDiagnostic("",
                     locale,
                     LocalizedNumberResult::Status::Incomplete,
                     NumericDiagnosticKind::ExpectedDigit);
    expectDiagnostic("-",
                     locale,
                     LocalizedNumberResult::Status::Incomplete,
                     NumericDiagnosticKind::IncompleteSign);
    expectDiagnostic("1.",
                     locale,
                     LocalizedNumberResult::Status::Incomplete,
    ...
    expectDiagnostic("1e",
                     locale,
                     LocalizedNumberResult::Status::Incomplete,
                     NumericDiagnosticKind::IncompleteExponent);
    expectDiagnostic("1,5e+",
                     german,
                     LocalizedNumberResult::Status::Incomplete,
                     NumericDiagnosticKind::IncompleteExponent);
    ...
    const auto decimalOnly =
        scanLocalizedNumber("1.", cLocaleContext(), NumericSyntaxContext::Standalone);
    EXPECT_EQ(decimalOnly.consumedBytes, 2U);
    EXPECT_EQ(decimalOnly.canonicalText, "1.");
    const auto exponentOnly =
        scanLocalizedNumber("1e+", cLocaleContext(), NumericSyntaxContext::Standalone);
...
TEST(NumericInputTest, RejectsOutOfRangeValues) {
    expectDiagnostic("1e999",
                     cLocaleContext(),
                     LocalizedNumberResult::Status::Invalid,
                     NumericDiagnosticKind::OutOfRange);
    expectDiagnostic("1,5e999",
                     expectContext("de_DE"),
                     LocalizedNumberResult::Status::Invalid,
                     NumericDiagnosticKind::OutOfRange);
}
