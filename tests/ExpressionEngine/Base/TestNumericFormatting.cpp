...
namespace
{
    ...
    void expectInvalidIdentifier(const std::string_view identifier)
    {
        ...
        EXPECT_EQ(result.error().code, NumericLocaleErrorCode::InvalidIdentifier) << std::string(identifier);
    ...
    void expectUnsupportedLocale(const std::string_view identifier)
    {
        const auto result = createNumericLocaleContext(identifier);
        ASSERT_FALSE(result.has_value()) << "区域 " << std::string(identifier) << " 不应构造成功";
        EXPECT_EQ(result.error().code, NumericLocaleErrorCode::UnsupportedLocale) << std::string(identifier);
        // 文案要指出是哪个标识符不受支持，并给出替代做法
        EXPECT_NE(result.error().message.find(std::string(identifier)), std::string::npos) << result.error().message;
        EXPECT_NE(result.error().message.find("NumericLocaleContext"), std::string::npos) << result.error().message;
    }
...
TEST(NumericFormattingTest, LocaleIdentifierSpellingsNormalizeToSameContext)
{
    ...
    for (const std::string_view variant: {"de-DE", "de_DE.UTF-8", "de-DE-1996", "DE_de", "de_de"})
    {
...
TEST(NumericFormattingTest, CLocaleAliasesAllResolveToCContext)
{
    ...
    for (const std::string_view alias: {"C", "c", "POSIX", "posix", "C.UTF-8", "C.utf8", "POSIX.utf8"})
    {
...
TEST(NumericFormattingTest, NotationAndPrecisionAreHonored)
{
    ...
    EXPECT_EQ(formatWith(1234.5, makeFormat(NumberNotation::Fixed, 3), cLocaleContext()), "1234.500");
    EXPECT_EQ(formatWith(1234.5678, makeFormat(NumberNotation::Fixed, 0), cLocaleContext()), "1235");

    // Scientific 的尾数按精度取小数位，小数点换成区域写法，指数标记保持 ASCII
    const std::string scientific = formatWith(12345.0, makeFormat(NumberNotation::Scientific, 2), german);
    ...
    EXPECT_EQ(formatWith(1234567.0, makeFormat(NumberNotation::Default, 4), cLocaleContext()).rfind("1.235e", 0), 0U);
...
TEST(NumericFormattingTest, HostProvidedZeroDigitLocalizesOutputDigits)
{
    ...
    EXPECT_EQ(formatWith(12.5, makeFormat(NumberNotation::Fixed, 1), arabicIndic), "\u0661\u0662.\u0665");
}
...
TEST(NumericFormattingTest, EmptyDecimalSeparatorThrowsInvalidArgument)
{
    ...
    EXPECT_THROW(static_cast<void>(formatWith(1.0, makeFormat(NumberNotation::Fixed, 2), broken)), std::invalid_argument);
}
