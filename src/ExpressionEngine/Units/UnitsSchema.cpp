...
std::string UnitsSchema::translate(const Quantity& quant,
    ...
    if (unitSpecification == candidates.end()) {
        throw Base::ExpressionError(
            std::format("单位方案 {} 的 {} 换算表里没有匹配条目，也没有阈值 0 "
                        "的兜底条目，请补一条兜底条目后重试",
                        m_spec.name,
                        unitTypeName));
    }
    ...
    if (unitSpecification->factor == 0) {
        ...
        return UnitsSchemasData::runSpecial(unitSpecification->unitString,
                                            value,
                                            static_cast<std::size_t>(format.getPrecision()),
                                            static_cast<std::size_t>(format.getDenominator()),
                                            factor,
                                            unitString);
    }
...
std::string UnitsSchema::toLocale(const Quantity& quant,
                                  const Base::NumericLocaleContext& formatting,
                                  const double factor,
    ...
    const std::string valueString =
        std::isfinite(convertedValue)
            ? formatNumericValue(convertedValue,
                                 format.getPrecision(),
                                 format.format,
                                 (format.option & QuantityFormat::OmitGroupSeparator) != 0,
                                 formatting)
            : std::to_string(convertedValue);

    // 角度与英制的单位符号是上标式记号，与数值之间不留空格才符合书写习惯
    const auto needsSeparator = [](const std::string& unit) {
        return !unit.empty() && unit != "°" && unit != "″" && unit != "′" && unit != "\"" &&
               unit != "'";
    };
