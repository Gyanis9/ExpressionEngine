...
UnitsSchemas::UnitsSchemas(const UnitsSchemasDataPack& pack)
    : m_pack{pack}, m_currentSchema{std::make_unique<UnitsSchema>(spec())},
      m_fractionDenominator{pack.defDenominator} {
}
...
UnitsSchemas::collect(const std::function<std::string(UnitsSchemaSpec)>& projector) {
    // 对外列表按方案编号排序，保证调用方看到的顺序与编号一致
    auto sortedSpecs = m_pack.specs;
    std::sort(sortedSpecs.begin(),
              sortedSpecs.end(),
              [](const UnitsSchemaSpec& left, const UnitsSchemaSpec& right) {
...
std::vector<std::string> UnitsSchemas::descriptions() {
    ...
    return collect([](const UnitsSchemaSpec& spec) {
        return spec.description == nullptr ? std::string{} : std::string{spec.description};
    });
}
