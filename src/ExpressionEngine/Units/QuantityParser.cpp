...
constexpr std::array functionTokenSpecs{
    FunctionTokenSpec{"acos", FunctionId::Acos},
    FunctionTokenSpec{"asin", FunctionId::Asin},
    FunctionTokenSpec{"atan", FunctionId::Atan},
    FunctionTokenSpec{"cos", FunctionId::Cos},
    FunctionTokenSpec{"exp", FunctionId::Exp},
    FunctionTokenSpec{"abs", FunctionId::Abs},
    FunctionTokenSpec{"log", FunctionId::Log},
    FunctionTokenSpec{"log10", FunctionId::Log10},
    FunctionTokenSpec{"sin", FunctionId::Sin},
    FunctionTokenSpec{"sinh", FunctionId::Sinh},
    FunctionTokenSpec{"tan", FunctionId::Tan},
    FunctionTokenSpec{"tanh", FunctionId::Tanh},
...
[[nodiscard]] std::size_t matchNumberWithDotInteger(const std::string_view text,
...
/// 数字形态二：允许以 '.' 开头，如 ".5"
[[nodiscard]] std::size_t matchNumberWithLeadingDot(const std::string_view text,
                                                    const std::size_t position) {
...
[[nodiscard]] std::size_t matchNumberWithCommaInteger(const std::string_view text,
                                                      const std::size_t position) {
    const auto leadingDigits = matchDigits(text, position);
    if (leadingDigits == 0) {
        return 0;
...
/// 数字形态四：允许以 ',' 开头，如 ",5"
[[nodiscard]] std::size_t matchNumberWithLeadingComma(const std::string_view text,
                                                      const std::size_t position) {
    std::size_t cursor = position;
    if (cursor < text.size() && text[cursor] == ',') {
        ++cursor;
...
[[nodiscard]] std::size_t longestNumberMatch(const std::string_view text,
                                             const std::size_t position,
...
[[nodiscard]] double convertNumberText(const std::string_view text, const char decimalSeparator) {
    ...
    double value = 0.0;
    const auto result =
        std::from_chars(canonical.data(), canonical.data() + canonical.size(), value);
    if (result.ec == std::errc::result_out_of_range) {
        throw Base::ParserError(std::format(
            "数量文本里的数字 {} 超出双精度可表示范围，请改用量级更小的写法", canonical));
    }
    ...
    while (m_position < m_text.size()) {
        ...
        if (character == '[') {
            ...
            if (closing == std::string_view::npos) {
                throw Base::ParserError(std::format(
                    "数量文本第 {} 个字符处的方括号注释没有闭合，请补上 ']'", m_position + 1));
            }
...
Token QuantityLexer::next() {
    ...
    if (numberLength != 0) {
        ...
        token.value =
            Quantity(convertNumberText(remaining.substr(0, numberLength), decimalSeparator));
    ...
    const auto bestLength =
        std::max({matchedUnitLength, matchedFunctionLength, matchedConstantLength});
    if (bestLength != 0) {
    ...
    throw Base::ParserError(
        std::format("数量文本第 {} 个字符处出现无法识别的字符 '{}'，请检查是否多写了符号",
                    m_position + 1,
                    remaining.front()));
}
...
void QuantityParserImpl::expect(const TokenKind kind, const std::string_view description) {
    if (m_current.kind != kind) {
        throw Base::ParserError(std::format(
            "数量文本第 {} 个字符处缺少{}，请补上后重试", m_current.offset + 1, description));
    }
    advance();
}
...
void QuantityParserImpl::expectEnd() {
    if (m_current.kind != TokenKind::End) {
        throw Base::ParserError(std::format(
            "数量文本第 {} 个字符处出现多余内容，请检查表达式是否完整", m_current.offset + 1));
    }
}
...
Quantity QuantityParserImpl::parseInput() {
    ...
    while (m_current.kind != TokenKind::End) {
        if (++extraItemCount > 2) {
            throw Base::ParserError(
                std::format("数量文本第 {} 个字符处开始的分段过多，相邻数量最多三段（如 5' "
                            "6\"），请把其余内容并入前面的表达式",
                            m_current.offset + 1));
        }
...
Quantity QuantityParserImpl::parseItem(const bool requireUnit) {
    ...
    if (m_current.kind == TokenKind::Unit) {
        ...
        throw Base::ParserError(
            std::format("数量文本第 {} 个字符处缺少单位，相邻分段（如 5' 6\"）要求每一段都带单位",
                        m_current.offset + 1));
    }
...
Quantity QuantityParserImpl::parseNumberAtom() {
    ...
    default:
        throw Base::ParserError(std::format(
            "数量文本第 {} 个字符处需要数字、函数或左括号，请检查写法", m_current.offset + 1));
    }
}
...
Quantity QuantityParserImpl::parseUnitPower() {
    ...
    if (m_current.kind == TokenKind::Caret) {
        ...
        if (!exponent.isDimensionless()) {
            throw Base::ParserError(std::format(
                "单位幂次必须是无量纲的纯数字，当前写法在偏移 {} 处带了单位", m_current.offset));
        }
...
Quantity QuantityParserImpl::parseUnitAtom() {
    ...
    default:
        throw Base::ParserError(std::format(
            "数量文本第 {} 个字符处需要单位符号或左括号，请检查写法", m_current.offset + 1));
    }
}
}  // namespace
