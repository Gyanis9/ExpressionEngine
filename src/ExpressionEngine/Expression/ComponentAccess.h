...
namespace ExpressionEngine::Expression
{
     ...
     *          本函数负责把 Range 展开成序列取值（v[0:2] 得到一个序列）。
     *          Index 支持向量分量（[0] 取 x，负下标从末尾计数）、序列元素（list(1; 2)[0]）
     *          与文本字符（[i] 取第 i 个 UTF-8 字符），其余值类型报错，不做任何隐式转换。
     * @param value 基值
     * @param component 分量
     ...
     * @throws Base::IndexError 区间下标越界或步长为 0
     */
