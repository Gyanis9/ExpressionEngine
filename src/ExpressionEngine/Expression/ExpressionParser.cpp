...
[[nodiscard]] std::optional<BinaryOperatorInfo> binaryOperatorFor(const ExpressionTokenKind kind) {
    ...
    case ExpressionTokenKind::Plus:
        return BinaryOperatorInfo{
            OperatorExpression::Operator::Add, additiveBinding, additiveBinding + 1};
    case ExpressionTokenKind::Minus:
        return BinaryOperatorInfo{
            OperatorExpression::Operator::Subtract, additiveBinding, additiveBinding + 1};
    case ExpressionTokenKind::Star:
        return BinaryOperatorInfo{OperatorExpression::Operator::Multiply,
                                  multiplicativeBinding,
                                  multiplicativeBinding + 1};
    case ExpressionTokenKind::Slash:
        return BinaryOperatorInfo{
            OperatorExpression::Operator::Divide, multiplicativeBinding, multiplicativeBinding + 1};
    case ExpressionTokenKind::Percent:
        return BinaryOperatorInfo{
            OperatorExpression::Operator::Modulo, multiplicativeBinding, multiplicativeBinding + 1};
    ...
    case ExpressionTokenKind::Equal:
        return BinaryOperatorInfo{
            OperatorExpression::Operator::Equal, comparisonBinding, comparisonBinding + 1};
    case ExpressionTokenKind::NotEqual:
        return BinaryOperatorInfo{
            OperatorExpression::Operator::NotEqual, comparisonBinding, comparisonBinding + 1};
    case ExpressionTokenKind::Less:
        return BinaryOperatorInfo{
            OperatorExpression::Operator::Less, comparisonBinding, comparisonBinding + 1};
    case ExpressionTokenKind::Greater:
        return BinaryOperatorInfo{
            OperatorExpression::Operator::Greater, comparisonBinding, comparisonBinding + 1};
    case ExpressionTokenKind::LessEqual:
        return BinaryOperatorInfo{
            OperatorExpression::Operator::LessEqual, comparisonBinding, comparisonBinding + 1};
    case ExpressionTokenKind::GreaterEqual:
        return BinaryOperatorInfo{
            OperatorExpression::Operator::GreaterEqual, comparisonBinding, comparisonBinding + 1};
    default:
        return std::nullopt;
    }
}
...
public:
    ExpressionParserImpl(IObjectResolver* resolver, const std::string_view text)
...
private:
    ...
    [[nodiscard]] ExpressionPtr
    makeBinary(OperatorExpression::Operator operation, ExpressionPtr left, ExpressionPtr right);
    [[nodiscard]] ExpressionPtr makeUnary(OperatorExpression::Operator operation,
...
bool ExpressionParserImpl::startsUnit() const {
    return m_current.kind == ExpressionTokenKind::Unit ||
           m_current.kind == ExpressionTokenKind::UsUnit;
}

void ExpressionParserImpl::expect(const ExpressionTokenKind kind,
                                  const std::string_view description) {
    if (m_current.kind != kind) {
        throw Base::ParserError(
            std::format("{}：缺少{}，请补上后重试", locationOf(m_current), description));
    }
    advance();
}

std::string ExpressionParserImpl::takeIdentifierLike(const std::string_view description) {
    if (m_current.kind != ExpressionTokenKind::Identifier &&
        m_current.kind != ExpressionTokenKind::CellAddress &&
        m_current.kind != ExpressionTokenKind::String) {
        throw Base::ParserError(
            std::format("{}：需要{}，请检查引用路径的写法", locationOf(m_current), description));
    }

    std::string text = m_current.kind == ExpressionTokenKind::String
                           ? unquoteReference(m_current.text)
                           : m_current.text;
    advance();
    return text;
}
...
ExpressionPtr ExpressionParserImpl::makeBinary(const OperatorExpression::Operator operation,
                                               ExpressionPtr left,
                                               ExpressionPtr right) {
    return std::make_unique<OperatorExpression>(
        m_resolver, std::move(left), operation, std::move(right));
}

ExpressionPtr ExpressionParserImpl::makeUnary(const OperatorExpression::Operator operation,
                                              ExpressionPtr operand) {
...
ExpressionPtr ExpressionParserImpl::parseDocument() {
    ...
    if (m_current.kind != ExpressionTokenKind::End) {
        throw Base::ParserError(
            std::format("{}：表达式后还有多余内容，请检查是否漏写运算符", locationOf(m_current)));
    }
...
ExpressionPtr ExpressionParserImpl::parseExpression(const int minimumBinding) {
    ...
    for (;;) {
        ...
        if (startsUnit() && unitPostfixBinding >= minimumBinding) {
            ...
            left = makeBinary(
                OperatorExpression::Operator::Multiply, std::move(left), parseUnitExpression());
            continue;
        }

        // 英制两段：5' 6" 按「第一段 + 第二段」计算，与 Expression.y 的 USUNIT 规则一致
        if (lastAttachmentWasImperial &&
            (m_current.kind == ExpressionTokenKind::Number ||
             m_current.kind == ExpressionTokenKind::Integer) &&
            m_next.kind == ExpressionTokenKind::UsUnit) {
            ExpressionPtr segment = makeNumber(m_current.kind == ExpressionTokenKind::Integer
                                                   ? static_cast<double>(m_current.integerValue)
                                                   : m_current.numberValue);
            advance();
            segment = makeBinary(
                OperatorExpression::Operator::Multiply, std::move(segment), parseUnitExpression());
            left =
                makeBinary(OperatorExpression::Operator::Add, std::move(left), std::move(segment));
        ...
        if (m_current.kind == ExpressionTokenKind::Question && ternaryBinding >= minimumBinding) {
            ...
            left = std::make_unique<ConditionalExpression>(
                m_resolver, std::move(left), std::move(trueBranch), std::move(falseBranch));
            lastAttachmentWasImperial = false;
            continue;
        }
...
ExpressionPtr ExpressionParserImpl::parseAtom() {
    ...
    case ExpressionTokenKind::Constant: {
        ...
        if (name == "pi") {
            return std::make_unique<ConstantExpression>(
                m_resolver, name, Units::Quantity(std::numbers::pi));
        }
        if (name == "e") {
            return std::make_unique<ConstantExpression>(
                m_resolver, name, Units::Quantity(std::numbers::e));
        }
    ...
    case ExpressionTokenKind::Dot:
        return parseReference();
    default:
        throw Base::ParserError(std::format("{}：需要数字、文本、函数、引用或左括号，请检查写法",
                                            locationOf(m_current)));
    }
}
...
ExpressionPtr ExpressionParserImpl::parseReference() {
    ...
    } else if (m_current.kind == ExpressionTokenKind::String &&
               m_next.kind == ExpressionTokenKind::Dot) {
        // <<文档>>.对象.属性
...
Expression::Component ExpressionParserImpl::parseIndexer() {
    ...
    if (m_current.kind == ExpressionTokenKind::Colon) {
        ...
        if (m_current.kind != ExpressionTokenKind::RightBracket &&
            m_current.kind != ExpressionTokenKind::Colon) {
    ...
    if (isRange) {
        return Expression::Component::rangeComponent(
            std::move(begin), std::move(end), std::move(step));
    }
...
ExpressionPtr ExpressionParserImpl::parseFunctionCall(std::string name) {
    ...
    if (m_current.kind != ExpressionTokenKind::RightParen) {
        for (;;) {
            ...
            if (m_current.kind == ExpressionTokenKind::Comma ||
                m_current.kind == ExpressionTokenKind::Semicolon) {
                advance();
                continue;
            }
            break;
    ...
    return std::make_unique<FunctionExpression>(
        m_resolver, function, std::string{name}, std::move(arguments));
}
...
ExpressionPtr ExpressionParserImpl::parseArgument() {
    ...
    if ((m_current.kind == ExpressionTokenKind::Identifier ||
         m_current.kind == ExpressionTokenKind::CellAddress) &&
        m_next.kind == ExpressionTokenKind::Colon) {
...
ExpressionPtr ExpressionParserImpl::parseUnitExpression() {
    ...
    while (m_current.kind == ExpressionTokenKind::Star ||
           m_current.kind == ExpressionTokenKind::Slash) {
        const bool isMultiplication = m_current.kind == ExpressionTokenKind::Star;
        advance();
        value = makeBinary(isMultiplication ? OperatorExpression::Operator::Multiply
                                            : OperatorExpression::Operator::Divide,
                           std::move(value),
                           parseUnitPower());
    }
...
ExpressionPtr ExpressionParserImpl::parseUnitPower() {
    ...
    if (m_current.kind == ExpressionTokenKind::Caret) {
        ...
        if (m_current.kind != ExpressionTokenKind::Integer &&
            m_current.kind != ExpressionTokenKind::Number) {
            throw Base::ParserError(
                std::format("{}：单位幂次需要整数，请把指数写成整数", locationOf(m_current)));
        }

        const double exponentValue = m_current.kind == ExpressionTokenKind::Integer
                                         ? static_cast<double>(m_current.integerValue)
                                         : m_current.numberValue;
        advance();

        base = makeBinary(OperatorExpression::Operator::Power,
                          std::move(base),
                          makeNumber(isNegativeExponent ? -exponentValue : exponentValue));
    }
...
ExpressionPtr ExpressionParserImpl::parseUnitAtom() {
    if (m_current.kind == ExpressionTokenKind::Unit ||
        m_current.kind == ExpressionTokenKind::UsUnit) {
        ...
        if (unitQuantity == nullptr) {
            throw Base::ParserError(
                std::format("表达式里的单位 '{}' 不在单位表中，请改用受支持的单位符号", symbol));
        }
    ...
    throw Base::ParserError(
        std::format("{}：这里需要单位符号或左括号，请检查写法", locationOf(m_current)));
}
