...
CellAddress::CellAddress(int row, int column, bool absoluteRow, bool absoluteColumn)
...
unsigned int CellAddress::asInteger() const noexcept {
    ...
    const unsigned int columnPart =
        static_cast<unsigned int>(static_cast<unsigned short>(m_column));
...
CellAddress stringToAddress(std::string_view address, bool silent) {
    ...
    if (row < 0 || column < 0) {
        ...
        throw Base::ParserError(std::format(
            "'{}' 不是合法的单元格地址，合法写法形如 A1、$B$2（列标 A 到 ZZ，行号 1 到 {}）；"
            "请检查是否漏写行号或用了小写列标",
            address,
...
int decodeColumn(std::string_view columnText, bool silent) {
    ...
    throw Base::IndexError(std::format(
        "列标 '{}' 不是合法列，合法范围是 A 到 ZZ；请改成大写列标，如 A、B、AA", columnText));
}
...
int decodeRow(std::string_view rowText, bool silent) {
    ...
    throw Base::IndexError(
        std::format("行号 '{}' 不是合法行，合法范围是 1 到 {}；请改成十进制行号，如 1、2",
                    rowText,
                    CellAddress::s_maxRows));
}
...
Range::Range(std::string_view rangeText, bool normalize) {
    ...
    const std::string_view beginText =
        separator == std::string_view::npos ? rangeText : rangeText.substr(0, separator);
    const std::string_view endText =
        separator == std::string_view::npos ? rangeText : rangeText.substr(separator + 1);
...
Range::Range(int rowBegin, int columnBegin, int rowEnd, int columnEnd, bool normalize)
    : m_rowCurrent(rowBegin), m_columnCurrent(columnBegin), m_rowBegin(rowBegin),
      m_columnBegin(columnBegin), m_rowEnd(rowEnd), m_columnEnd(columnEnd) {
    if (normalize) {
        this->normalize();
    }
...
Range::Range(const CellAddress& from, const CellAddress& to, bool normalize)
    : m_rowCurrent(from.row()), m_columnCurrent(from.column()), m_rowBegin(from.row()),
      m_columnBegin(from.column()), m_rowEnd(to.row()), m_columnEnd(to.column()) {
    if (normalize) {
        this->normalize();
    }
    m_rowCurrent = m_rowBegin;
