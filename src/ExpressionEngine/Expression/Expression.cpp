...
bool essentiallyInteger(double value, long& result) {
    ...
    if (integralPart < static_cast<double>(std::numeric_limits<long>::min()) ||
        integralPart > static_cast<double>(std::numeric_limits<long>::max())) {
        return false;
    }
...
/// 参数个数不符时的统一报错
[[noreturn]] void throwArgumentCount(std::string_view label,
                                     std::string_view requirement,
                                     std::size_t argumentCount) {
    throw EvaluationError(
        std::format("{}() 的参数个数不正确：需要{}，实际收到 {} 个；请调整调用处的参数个数",
                    label,
                    requirement,
                    argumentCount));
}
 ...
 */
void validateFunctionCall(FunctionExpression::Function function,
                          std::size_t argumentCount,
    ...
    case Function::Last:
        throw Base::ParserError(std::format(
            "'{}' 不是可求值的函数名；请改用函数表里的函数名（如 sqrt、sin、mrotatez）", label));
    default:
        throw EvaluationError(
            std::format("函数 '{}' 未登记参数个数要求；请同步更新函数表与参数校验", label));
    }
}
...
/// 取第 index 个实参的向量取值
Base::Vector3d vectorArgument(const std::vector<ExpressionPtr>& arguments,
                              std::size_t index,
                              std::string_view label) {
    ...
    throw Base::TypeError(
        std::format("{}() 的第 {} 个参数需要向量，实际是{}；请改用 vector(x, y, z) 构造向量",
                    label,
                    index + 1,
                    valueTypeName(value)));
}
...
double lengthArgument(const std::vector<ExpressionPtr>& arguments,
                      std::size_t index,
                      std::string_view label) {
    const Units::Quantity quantity = toQuantity(
        arguments[index]->evaluate(), std::format("{}() 的第 {} 个参数", label, index + 1));
    if (!quantity.isDimensionlessOrUnit(Units::Unit::Length)) {
        throw Base::UnitsMismatchError(
            std::format("{}() 的第 {} 个参数需要长度量或纯数，实际是 {}；"
                        "请改用 mm、in 这类长度单位",
                        label,
                        index + 1,
...
/// 取第 index 个实参的纯数值
double numberArgument(const std::vector<ExpressionPtr>& arguments,
                      std::size_t index,
                      std::string_view label) {
    return toDouble(arguments[index]->evaluate(),
                    std::format("{}() 的第 {} 个参数", label, index + 1));
}

/// 取第二个向量实参：两参数形式直接给向量，四参数形式给三个分量
Base::Vector3d secondVectorArgument(const std::vector<ExpressionPtr>& arguments,
                                    std::string_view label) {
    if (arguments.size() == 2) {
        return vectorArgument(arguments, 1, label);
    }
    return Base::Vector3d(numberArgument(arguments, 1, label),
                          numberArgument(arguments, 2, label),
                          numberArgument(arguments, 3, label));
}

/// 对第一个实参施加变换矩阵，支持矩阵、位姿与旋转
Value transformFirstArgument(const std::vector<ExpressionPtr>& arguments,
...
double angleArgument(const Value& value, std::string_view context) {
    ...
    if (!quantity.isDimensionlessOrUnit(Units::Unit::Angle)) {
        throw Base::UnitsMismatchError(
            std::format("{}需要角度量或纯数，实际是 {}；请改用 deg、rad 这类角度单位",
                        context,
                        quantity.getUserString()));
    }
...
Value addValues(const Value& left, const Value& right) {
    if (const auto* leftText = std::get_if<std::string>(&left)) {
        ...
        if (rightText == nullptr) {
            throw Base::TypeError(std::format(
                "加法不能用文本与{}相加；请统一两侧为文本，或都换成数值", valueTypeName(right)));
        }
...
Value moduloValues(const Value& left, const Value& right) {
    ...
    if (leftQuantity.getUnit() != rightQuantity.getUnit() && !leftQuantity.isDimensionless() &&
        !rightQuantity.isDimensionless()) {
        throw Base::UnitsMismatchError(
            std::format("取余要求两侧单位一致或同为纯数：左侧是 {}，右侧是 {}；"
                        "请先换算成同一单位",
                        leftQuantity.getUnit().getString(),
                        rightQuantity.getUnit().getString()));
    }
    if (rightQuantity.getValue() == 0.0) {
        throw Base::ValueError("取余的模数为零，无法求值；请检查表达式里的模数");
    }
    return Value(Units::Quantity(std::fmod(leftQuantity.getValue(), rightQuantity.getValue()),
                                 leftQuantity.getUnit()));
}
...
Value negateValue(const Value& value) {
    ...
    throw Base::TypeError(
        std::format("取负运算符不能作用于{}；请改用数值或向量，反向的几何变换请用 minvert()",
                    valueTypeName(value)));
}
    ...
    [[nodiscard]] Units::Quantity getQuantity() const override {
        if (m_count == 0) {
            ...
            throw Base::ValueError(
                "average() 至少需要一个数值参数；请检查参数是否为数值属性或数量");
        }
...
public:
    ...
    [[nodiscard]] Units::Quantity getQuantity() const override {
        ...
        return Units::Quantity(
            (m_squaredDeviationSum / (m_count - 1.0)).pow(Units::Quantity(0.5)).getValue(),
            m_mean.getUnit());
    }
...
/// 收集区间里各单元格的取值
void collectRangeCells(const Expression& context,
                       const RangeExpression& rangeExpression,
                       Collector& collector) {
    ...
    do {
        ...
        if (!value.has_value()) {
            throw Base::AttributeError(std::format(
                "单元格 {} 尚未赋值，聚合函数无法取值；请先给该单元格赋值", range.address()));
        }
...
/// 按聚合函数种类收集所有实参
Value collectAggregate(const Expression& context,
...
Expression::Component::Component(const Component& other)
    : kind(other.kind), name(other.name), index(other.index ? other.index->copy() : nullptr),
...
Expression::Component Expression::Component::rangeComponent(ExpressionPtr begin,
                                                            ExpressionPtr end,
...
bool Expression::Component::isSame(const Component& other) const {
    ...
    return expressionsSame(index.get(), other.index.get()) &&
           expressionsSame(endIndex.get(), other.endIndex.get()) &&
           expressionsSame(step.get(), other.step.get());
}
...
Value Expression::evaluate() const {
    if (!m_components.empty()) {
        // 分量模型留给后续 Identifier 模块；静默忽略会让引用悄悄取到错误的值，
        // 因此在节点求值之前拦下，避免连宿主属性都不必要地读一遍
        throw EvaluationError(std::format("表达式 {} 带有分量或下标，本阶段尚未支持；"
                                          "请先改用整体属性引用，分量访问将由 Identifier 模块提供",
                                          toString(true)));
    }
    return evaluateNode();
}
...
ExpressionPtr makeValueExpression(IObjectResolver* resolver, const Value& value) {
    ...
    if (const auto* boolean = std::get_if<bool>(&value)) {
        return std::make_unique<ConstantExpression>(
            resolver, *boolean ? "True" : "False", Units::Quantity(*boolean ? 1.0 : 0.0));
    }
...
UnitExpression::UnitExpression(IObjectResolver* resolver,
                               const Units::Quantity& quantity,
...
ConstantExpression::ConstantExpression(IObjectResolver* resolver,
                                       std::string name,
                                       const Units::Quantity& quantity)
...
OperatorExpression::OperatorExpression(IObjectResolver* resolver,
                                       ExpressionPtr left,
                                       Operator operation,
                                       ExpressionPtr right)
    : UnitExpression(resolver), m_operator(operation), m_left(std::move(left)),
    ...
    if (isUnary) {
        if (m_right != nullptr) {
            throw EvaluationError(
                "一元运算符不接受右操作数；请把右操作数留空，二元运算请改用对应的运算符");
        }
        return;
    }
    if (m_right == nullptr) {
...
ExpressionPtr OperatorExpression::simplify() const {
    ...
    if (m_operator == Operator::Negate || m_operator == Operator::Positive) {
        ...
        return std::make_unique<OperatorExpression>(
            resolver(), std::move(left), m_operator, nullptr);
    }
    ...
    return std::make_unique<OperatorExpression>(
        resolver(), std::move(left), m_operator, std::move(right));
}
...
Value OperatorExpression::evaluateNode() const {
    ...
    case Operator::Positive: {
        ...
        throw Base::TypeError(
            std::format("取正运算符不能作用于{}；请改用数值或向量", valueTypeName(value)));
    }
    ...
    default:
        throw EvaluationError(
            std::format("运算符 '{}' 不支持求值；请检查表达式构造过程", operatorText(m_operator)));
    }
}
...
void OperatorExpression::appendText(std::string& text, bool persistent, int) const {
    ...
    default:
        ...
        throw EvaluationError(std::format("运算符 '{}' 没有文本写法；请检查表达式构造过程",
...
ExpressionPtr OperatorExpression::copyNode() const {
    return std::make_unique<OperatorExpression>(
        resolver(), m_left->copy(), m_operator, m_right ? m_right->copy() : nullptr);
}
...
ConditionalExpression::ConditionalExpression(IObjectResolver* resolver,
                                             ExpressionPtr condition,
                                             ExpressionPtr trueExpression,
                                             ExpressionPtr falseExpression)
    : Expression(resolver), m_condition(std::move(condition)),
      m_trueExpression(std::move(trueExpression)), m_falseExpression(std::move(falseExpression)) {
    if (m_condition == nullptr || m_trueExpression == nullptr || m_falseExpression == nullptr) {
        throw EvaluationError(
            "条件表达式需要条件、真分支与假分支三个子表达式；请检查表达式构造过程");
    }
}
...
ExpressionPtr ConditionalExpression::simplify() const {
    ...
    return std::make_unique<ConditionalExpression>(resolver(),
                                                   std::move(simplifiedCondition),
                                                   m_trueExpression->simplify(),
                                                   m_falseExpression->simplify());
}
...
ExpressionPtr ConditionalExpression::copyNode() const {
    return std::make_unique<ConditionalExpression>(
        resolver(), m_condition->copy(), m_trueExpression->copy(), m_falseExpression->copy());
}
...
FunctionExpression::FunctionExpression(IObjectResolver* resolver,
                                       Function function,
                                       std::string name,
                                       std::vector<ExpressionPtr> arguments)
    : UnitExpression(resolver), m_function(function), m_name(std::move(name)),
    ...
    const std::string_view label =
        m_name.empty() ? functionName(m_function) : std::string_view(m_name);
...
ExpressionPtr FunctionExpression::simplify() const {
    ...
    return std::make_unique<FunctionExpression>(
        resolver(), m_function, m_name, std::move(simplifiedArguments));
}
...
Value FunctionExpression::evaluateAggregate(const Expression& context,
                                            Function function,
                                            const std::vector<ExpressionPtr>& arguments) {
    return collectAggregate(context, function, arguments);
}

Value FunctionExpression::evaluateFunction(const Expression& context,
                                           Function function,
                                           const std::vector<ExpressionPtr>& arguments) {
    using Function = FunctionExpression::Function;
    ...
    if (arguments.empty()) {
        throw EvaluationError(
            std::format("{}() 至少需要 1 个参数；请补上实参", functionName(function)));
    }
    ...
    case Function::MatrixInvert: {
        ...
        if (const auto* matrix = std::get_if<Base::Matrix4D>(&target)) {
            if (std::fabs(matrix->determinant()) <= std::numeric_limits<double>::epsilon()) {
                throw Base::ValueError(
                    "minvert() 的矩阵不可逆（行列式接近 0）；请检查矩阵是否退化，"
                    "或改用可逆的构造方式");
            }
    ...
    case Function::MatrixRotate: {
        ...
        if (arguments.size() == 4) {
            ...
            rotation.setYawPitchRoll(numberArgument(arguments, 1, label),
                                     numberArgument(arguments, 2, label),
                                     numberArgument(arguments, 3, label));
        } else {
            ...
            if (const auto* given = std::get_if<Base::Rotation>(&second)) {
                ...
                if (const auto* endDirection = std::get_if<Base::Vector3d>(&third)) {
                    ...
                    rotation =
                        Base::Rotation(*axis, angleArgument(third, "mrotate() 的第三个参数"));
                }
            } else {
                throw Base::TypeError(
                    std::format("mrotate() 的第二个参数需要旋转、矩阵或轴向量，实际是{}；"
    ...
    case Function::MatrixRotateZ: {
        const double angle =
            angleArgument(arguments[1]->evaluate(), std::format("{}() 的第二个参数", label));
        const Base::Rotation rotation(
            Base::Vector3d(function == Function::MatrixRotateX ? 1.0 : 0.0,
                           function == Function::MatrixRotateY ? 1.0 : 0.0,
    ...
    case Function::Matrix: {
        ...
        return Base::Matrix4D(elements[0],
                              elements[1],
                              elements[2],
                              elements[3],
                              elements[4],
    ...
    case Function::Placement: {
        ...
        if (arguments.size() == 1) {
            ...
            throw Base::TypeError(
                std::format("placement() 只给一个参数时需要矩阵或位姿，实际是{}；"
    ...
    case Function::Rotation: {
        ...
        if (arguments.size() == 3) {
            // 三个数值按 yaw、pitch、roll 解释，与 FreeCAD 一致按弧度给出
            Base::Rotation rotation;
            rotation.setYawPitchRoll(numberArgument(arguments, 0, label),
                                     numberArgument(arguments, 1, label),
                                     numberArgument(arguments, 2, label));
            return rotation;
        }
    ...
    case Function::Vector:
        return Base::Vector3d(numberArgument(arguments, 0, label),
                              numberArgument(arguments, 1, label),
                              numberArgument(arguments, 2, label));
    case Function::Stringify:
    ...
    case Function::Address: {
        long row = 0;
        long column = 0;
        const Units::Quantity rowQuantity =
            toQuantity(arguments[0]->evaluate(), "address() 的第一个参数");
        const Units::Quantity columnQuantity =
            toQuantity(arguments[1]->evaluate(), "address() 的第二个参数");
        if (!rowQuantity.isDimensionless() || !columnQuantity.isDimensionless()) {
            throw Base::UnitsMismatchError("address() 的行号与列号必须是纯数；请去掉单位");
        }
        if (!essentiallyInteger(rowQuantity.getValue(), row) ||
            !essentiallyInteger(columnQuantity.getValue(), column)) {
        ...
        if (arguments.size() > 2) {
            ...
            const Units::Quantity typeQuantity =
                toQuantity(arguments[2]->evaluate(), "address() 的第三个参数");
            if (!typeQuantity.isDimensionless() ||
                !essentiallyInteger(typeQuantity.getValue(), referenceType)) {
        ...
        const CellAddress cell(
            static_cast<int>(row) - 1, static_cast<int>(column) - 1, absoluteRow, absoluteColumn);
        if (!cell.isValid()) {
            throw Base::ValueError(
                std::format("address() 给出的单元格（第 {} 行、第 {} 列）超出范围；"
                            "行号 1 到 {}，列号 1 到 {}",
                            row,
                            column,
                            CellAddress::s_maxRows,
                            CellAddress::s_maxColumns));
        }
    ...
    case Function::VectorScaleZ: {
        ...
        case Function::VectorScale:
            firstVector.Scale(lengthArgument(arguments, 1, label),
                              lengthArgument(arguments, 2, label),
                              lengthArgument(arguments, 3, label));
            return firstVector;
        case Function::VectorScaleX:
        ...
        switch (function) {
        case Function::VectorAngle:
            return Units::Quantity(toDegrees(firstVector.GetAngle(secondVector)),
                                   Units::Unit::Angle);
        case Function::VectorCross:
        ...
        switch (function) {
        case Function::VectorLineDistance:
            return Units::Quantity(firstVector.DistanceToLine(secondVector, thirdVector),
                                   Units::Unit::Length);
        case Function::VectorLineSegmentDistance:
        ...
        case Function::VectorLineProjection:
            ...
            if (thirdVector.Sqr() == 0.0) {
                throw Base::ValueError(
                    "vlineproj() 的直线方向是零向量，无法确定直线；请给出非零方向");
            }
            return secondVector +
                   (((firstVector - secondVector) * thirdVector) / thirdVector.Sqr()) * thirdVector;
        case Function::VectorPlaneDistance:
            return Units::Quantity(firstVector.DistanceToPlane(secondVector, thirdVector),
                                   Units::Unit::Length);
        case Function::VectorPlaneProjection:
    ...
    // 其余函数都按数量求值
    const Units::Quantity firstQuantity =
        toQuantity(arguments[0]->evaluate(), std::format("{}() 的第一个参数", label));
    std::optional<Units::Quantity> secondQuantity;
    std::optional<Units::Quantity> thirdQuantity;
    if (arguments.size() > 1) {
        secondQuantity =
            toQuantity(arguments[1]->evaluate(), std::format("{}() 的第二个参数", label));
    }
    if (arguments.size() > 2) {
        thirdQuantity =
            toQuantity(arguments[2]->evaluate(), std::format("{}() 的第三个参数", label));
    }
    if (arguments.size() > 3) {
    ...
    case Function::ArcTangent:
        if (!firstQuantity.isDimensionless()) {
            throw Base::UnitsMismatchError(
                std::format("{}() 的参数需要纯数（反三角函数取比值），实际是 {}；"
                            "请先除以同量纲的参照量",
                            label,
                            firstQuantity.getUserString()));
        }
    ...
    case Function::HyperbolicCosine:
        if (!firstQuantity.isDimensionless()) {
            throw Base::UnitsMismatchError(
                std::format("{}() 的参数需要纯数，实际是 {}；请先除以同量纲的参照量",
                            label,
                            firstQuantity.getUserString()));
        }
        unit = Units::Unit();
    ...
    case Function::ArcTangent2:
        if (firstQuantity.getUnit() != secondQuantity->getUnit()) {
            throw Base::UnitsMismatchError(
                std::format("atan2() 的两个参数单位必须一致：左侧是 {}，右侧是 {}；"
                            "请先换算成同一单位",
                            firstQuantity.getUnit().getString(),
                            secondQuantity->getUnit().getString()));
        }
        unit = Units::Unit::Angle;
        scaler = 180.0 / std::numbers::pi;
        break;
    case Function::Modulo:
        if (firstQuantity.getUnit() != secondQuantity->getUnit() &&
            !firstQuantity.isDimensionless() && !secondQuantity->isDimensionless()) {
            throw Base::UnitsMismatchError(
                std::format("mod() 要求两侧单位一致或同为纯数：左侧是 {}，右侧是 {}；"
                            "请先换算成同一单位",
                            firstQuantity.getUnit().getString(),
                            secondQuantity->getUnit().getString()));
        }
    ...
    case Function::Power: {
        if (!secondQuantity->isDimensionless()) {
            throw Base::UnitsMismatchError(
                std::format("pow() 的指数不能带量纲，实际是 {}；请改用纯数指数",
                            secondQuantity->getUserString()));
        }
        ...
        if (!firstQuantity.isDimensionless()) {
            ...
            if (std::fabs(exponent - std::round(exponent)) < 1e-9) {
                ...
                throw Base::UnitsMismatchError(
                    std::format("pow() 的底数带量纲时指数必须是整数，实际是 {}；"
                                "请把指数改成整数",
                                exponent));
            }
        }
    ...
    case Function::Cathetus:
        if (firstQuantity.getUnit() != secondQuantity->getUnit()) {
            throw Base::UnitsMismatchError(
                std::format("{}() 的各参数单位必须一致：第一个是 {}，第二个是 {}；"
                            "请先换算成同一单位",
                            label,
                            firstQuantity.getUnit().getString(),
                            secondQuantity->getUnit().getString()));
        ...
        if (arguments.size() > 2 && secondQuantity->getUnit() != thirdQuantity->getUnit()) {
            throw Base::UnitsMismatchError(
                std::format("{}() 的各参数单位必须一致：第二个是 {}，第三个是 {}；"
                            "请先换算成同一单位",
                            label,
                            secondQuantity->getUnit().getString(),
                            thirdQuantity->getUnit().getString()));
        }
        unit = firstQuantity.getUnit();
        break;
    case Function::TranslationMatrix:
        if (firstQuantity.isDimensionlessOrUnit(Units::Unit::Length) &&
            secondQuantity->isDimensionlessOrUnit(Units::Unit::Length) &&
    ...
    case Function::Hypotenuse:
        output = std::sqrt(
            std::pow(firstQuantity.getValue(), 2) + std::pow(secondQuantity->getValue(), 2) +
    ...
    case Function::RotationY:
    case Function::RotationZ:
        return Base::Rotation(Base::Vector3d(function == Function::RotationX ? 1.0 : 0.0,
                                             function == Function::RotationY ? 1.0 : 0.0,
                                             function == Function::RotationZ ? 1.0 : 0.0),
                              value);
    case Function::TranslationMatrix: {
        Base::Matrix4D matrix;
        matrix.move(
            firstQuantity.getValue(), secondQuantity->getValue(), thirdQuantity->getValue());
        return matrix;
    }
    case Function::LogicalNot:
...
ExpressionPtr FunctionExpression::copyNode() const {
    ...
    return std::make_unique<FunctionExpression>(
        resolver(), m_function, m_name, std::move(arguments));
}
...
std::string VariableExpression::pathText() const {
    ...
    return "<<" + m_reference.documentName + ">>." + m_reference.objectName + "." +
           m_reference.propertyName;
}
...
IProperty* VariableExpression::resolveProperty() const {
    ...
    if (objectResolver == nullptr) {
        throw Base::NameError(
            std::format("表达式引用了 '{}'，但表达式没有绑定对象解析器"
    ...
    if (object == nullptr) {
        throw Base::NameError(
            std::format("解析器找不到 '{}' 引用的对象（文档 '{}'、对象 '{}'）；"
                        "请检查对象名，或先用 IObjectResolver::objectNames() 列出可用对象",
                        path,
                        m_reference.documentName,
                        m_reference.objectName));
    }
    ...
    if (property == nullptr) {
        throw Base::AttributeError(std::format("对象 '{}' 上没有属性 '{}'（表达式引用 '{}'）；"
                                               ...
                                               m_reference.propertyName,
                                               path,
                                               joinNames(object->propertyNames())));
    }
    return property;
}
...
Value VariableExpression::evaluateNode() const {
    ...
    const std::optional<Value> value = property->value();
    if (!value.has_value()) {
        throw Base::AttributeError(
            std::format("属性 '{}' 尚未赋值，表达式无法取值；请先给该属性赋值", pathText()));
    }
    return *value;
}
