...
bool isGeometric(const Value& value) {
    return std::holds_alternative<Base::Vector3d>(value) ||
           std::holds_alternative<Base::Matrix4D>(value) ||
           std::holds_alternative<Base::Rotation>(value) ||
...
Units::Quantity toQuantity(const Value& value, std::string_view context) {
    ...
    throw Base::TypeError(
        std::format("{}需要数量，但拿到的是{}；几何值请改用 vdot()、vangle() 等向量函数"
                    "或取具体分量后再参与数值运算",
                    context,
                    valueTypeName(value)));
}
...
double toDouble(const Value& value, std::string_view context) {
    ...
    throw Base::TypeError(std::format(
        "{}需要纯数，但拿到的是{}；请改用数字或无量纲的量", context, valueTypeName(value)));
}
    ...
    return std::visit(
        [](const auto& leftValue, const auto& rightValue) -> bool {
            ...
            if constexpr (!std::is_same_v<LeftType, RightType>) {
                ...
                static_assert(std::is_same_v<LeftType, Base::Placement>,
                              "Value 增加了新的备选类型，请同步更新相等判定");
                return leftValue.getPosition().IsEqual(rightValue.getPosition(),
                                                       Base::Precision::Confusion()) &&
                       leftValue.getRotation().isSame(rightValue.getRotation(),
                                                      Base::Precision::Angular());
            }
        },
        left,
        right);
...
bool valueLessThan(const Value& left, const Value& right) {
    ...
    if (leftNumber.has_value() && rightNumber.has_value()) {
        if (leftNumber->getUnit() != rightNumber->getUnit()) {
            ...
            throw Base::UnitsMismatchError(
                std::format("比较两个量的大小时单位必须一致：左侧是 {}，右侧是 {}；"
                            "请先把两侧换算成同一单位",
                            leftNumber->getUnit().getString(),
                            rightNumber->getUnit().getString()));
        }
    ...
    if (const auto* leftText = std::get_if<std::string>(&left)) {
        ...
        if (rightText == nullptr) {
            throw Base::TypeError(std::format(
                "文本只能和文本比较大小，右侧是{}；请统一两侧的取值类型", valueTypeName(right)));
        }
    ...
    if (const auto* leftBoolean = std::get_if<bool>(&left)) {
        ...
        if (rightBoolean == nullptr) {
            throw Base::TypeError(std::format(
                "布尔只能和布尔比较大小，右侧是{}；请统一两侧的取值类型", valueTypeName(right)));
        }
