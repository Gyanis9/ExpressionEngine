...
constexpr bool startsWithUnicodeMinus(const std::string_view text, const std::size_t offset) {
    return text.size() - offset >= unicodeMinusSign.size() &&
           text.compare(offset, unicodeMinusSign.size(), unicodeMinusSign) == 0;
}
...
UnitMatch matchUnitSymbol(std::string_view text, const std::size_t offset) {
    UnitMatch best;
    const auto consider = [&best, text, offset](const std::string_view symbol,
                                                const ExpressionTokenKind kind) {
        if (symbol.size() > best.length && text.size() - offset >= symbol.size() &&
            text.compare(offset, symbol.size(), symbol) == 0) {
...
NumberMatch matchNumber(std::string_view text, const std::size_t offset) {
    ...
    if (integerDigits < text.size() - offset &&
        (text[offset + integerDigits] == '.' || text[offset + integerDigits] == ',')) {
...
double parseNumberValue(const std::string_view raw, const int column) {
    ...
    if (result.ptr != end) {
        ...
        throw Base::ParserError(
            std::format("数字 '{}' 无法解析，请检查表达式第 {} 列", raw, column));
    }
...
long long parseIntegerValue(const std::string_view raw, const int column) {
    ...
    if (result.ec == std::errc::result_out_of_range) {
        ...
        // 分支在本词法中不可达
        throw Base::OverflowError(
            std::format("整数 '{}' 超出 long long 能表示的范围，请改写成小数或缩小数值（第 {} 列）",
                        raw,
                        column));
    }
    if (result.ptr != raw.data() + raw.size()) {
        throw Base::ParserError(
            std::format("整数 '{}' 无法解析，请检查表达式第 {} 列", raw, column));
    }
    return value;
}
...
ConstantMatch matchConstant(std::string_view text, const std::size_t offset) {
    ...
    for (const ConstantSpec& spec : constantSpecs) {
        if (spec.literal.size() > best.length && text.size() - offset >= spec.literal.size() &&
            text.compare(offset, spec.literal.size(), spec.literal) == 0) {
...
StringMatch matchString(std::string_view text, const std::size_t offset, const int column) {
    ...
    throw Base::ParserError(
        std::format("字符串 << 没有配对的 >>，请补齐结束符（第 {} 列）", column));
}
...
ExpressionToken ExpressionLexer::next() {
    ...
    const auto consider = [&bestKind, &bestLength](const ExpressionTokenKind kind,
    ...
    const ExpressionTokenKind stringKind = stringMatch.documentReference
                                               ? ExpressionTokenKind::DocumentRef
    ...
    if (bestLength == 0) {
        ...
        if (characterText == "#" || characterText == "@" || characterText == "{" ||
            characterText == "}") {
            throw Base::ParserError(std::format(
                "字符 '{}' 不能单独出现在这里，请检查表达式第 {} 列", characterText, tokenColumn));
        }
        throw Base::ParserError(
            std::format("无法识别的字符 '{}'，请检查表达式第 {} 列", characterText, tokenColumn));
    }
