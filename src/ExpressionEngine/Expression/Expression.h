 ...
 *          IObjectResolver 解析，节点不持有对象树的所有权。分量列表在本阶段只保存与
 *          打印，求值遇到分量时会明确报错，不会静默忽略。
 */
class Expression {
public:
    /// 分量种类
    ...
    struct Component {
        ...
        static Component rangeComponent(ExpressionPtr begin,
                                        ExpressionPtr end,
                                        ExpressionPtr stepExpression = nullptr);
    ...
    toString(bool persistent = false, bool checkPriority = false, int indent = 0) const;
...
protected:
    /// 节点自身的求值；分量访问由 evaluate() 在外层统一处理
...
private:
    ...
    ComponentList m_components;   ///< 分量列表，本阶段只保存与打印
...
public:
     ...
     */
    explicit UnitExpression(IObjectResolver* resolver = nullptr,
                            const Units::Quantity& quantity = Units::Quantity(),
                            const std::string& unitText = std::string());
...
public:
     ...
     * @param quantity 数量
     */
    explicit NumberExpression(IObjectResolver* resolver = nullptr,
                              const Units::Quantity& quantity = Units::Quantity());
...
public:
     ...
     * @param quantity 常量取值
     */
    explicit ConstantExpression(IObjectResolver* resolver = nullptr,
...
public:
     ...
     */
    explicit OperatorExpression(IObjectResolver* resolver = nullptr,
...
public:
     ...
     */
    explicit ConditionalExpression(IObjectResolver* resolver = nullptr,
                                   ExpressionPtr condition = nullptr,
                                   ExpressionPtr trueExpression = nullptr,
...
public:
    ...
    explicit FunctionExpression(
        IObjectResolver* resolver = nullptr,
     ...
     */
    [[nodiscard]] static Value evaluateFunction(const Expression& context,
                                                Function function,
...
private:
     ...
     * @return 聚合结果
     */
    [[nodiscard]] static Value evaluateAggregate(const Expression& context,
                                                 Function function,
                                                 const std::vector<ExpressionPtr>& arguments);
 ...
 *          完整的标识符模型（数组下标、映射键、区间、<<Doc>>.Obj.Prop 文本形式）留给后续
 *          Identifier 模块，本阶段分量只保存不参与求值。
 */
class VariableExpression : public UnitExpression {
public:
    /// 引用路径的临时表示，供后续 Identifier 模块替换
    struct Reference {
     ...
     */
    explicit VariableExpression(IObjectResolver* resolver = nullptr,
...
public:
     ...
     * @param text 文本内容
     */
    explicit StringExpression(IObjectResolver* resolver = nullptr,
...
public:
     ...
     * @param end 结束单元格地址文本
     */
    explicit RangeExpression(IObjectResolver* resolver = nullptr,
