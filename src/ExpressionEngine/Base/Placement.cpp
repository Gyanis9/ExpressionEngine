...
Placement Placement::fromDualQuaternion(DualQuat qq) {
    ...
    return Placement(Vector3d(moveQuaternion.x.re, moveQuaternion.y.re, moveQuaternion.z.re),
                     rotation);
}
...
DualQuat Placement::toDualQuaternion() const {
    ...
    m_rotation.getValue(rotationQuaternion.x.re,
                        rotationQuaternion.y.re,
                        rotationQuaternion.z.re,
                        rotationQuaternion.w.re);
    // 对偶部按 0.5·t·r 编码
