...
public:
     ...
     */
    DualNumber(double reValue, double dualValue = 0.0) : re(reValue), du(dualValue) {
    }
 ...
 * @return 逐分量相加的结果
 */
inline DualNumber operator+(DualNumber left, DualNumber right) {
 ...
 * @return 实部相加、对偶部保持的结果
 */
inline DualNumber operator+(double left, DualNumber right) {
 ...
 */
inline DualNumber pow(DualNumber base, double power) {
