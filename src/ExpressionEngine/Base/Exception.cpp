...
Exception::Exception(std::string message, const std::source_location& location)
    ...
    : std::runtime_error(message), m_message(std::move(message)), m_file(location.file_name()),
      m_sourceLine(static_cast<int>(location.line())), m_function(location.function_name()) {
}
