...
namespace ExpressionEngine::Base
{
    namespace
    {
        ...
        static_assert(EulerSequenceNames.size() == static_cast<std::size_t>(Rotation::EulerSequenceLast) - 1, "欧拉序列名字表与 EulerSequence 枚举不同步");
    ...
    void Rotation::setValue(const Matrix4D &matrix)
    {
        ...
        } else
        {
            ...
            scale                   = 0.5 / scale;
            m_quaternion[3]         = (rotationMatrix[lastIndex][nextIndex] - rotationMatrix[nextIndex][lastIndex]) * scale;
            m_quaternion[nextIndex] = (rotationMatrix[nextIndex][mainIndex] + rotationMatrix[mainIndex][nextIndex]) * scale;
            m_quaternion[lastIndex] = (rotationMatrix[lastIndex][mainIndex] + rotationMatrix[mainIndex][lastIndex]) * scale;
        }
    ...
    Rotation Rotation::makeRotationByAxes(Vector3d xDirection, Vector3d yDirection, Vector3d zDirection, const char *priorityOrder)
    {
        ...
        {
            throw ValueError(std::format("makeRotationByAxes：优先级串长度为 {}，必须恰为 3，例如 \"ZXY\"", priorityView.size()));
        }

        std::array<int, 3> order{};
        for (int index = 0; index < 3; ++index)
        {
            order[static_cast<std::size_t>(index)] = priorityView[static_cast<std::size_t>(index)] - 'X';
            // 只接受大写的 X/Y/Z：'X'..'Z' 与 'X' 的差落在 0..2，小写字母与其它字符一律非法
            if (order[static_cast<std::size_t>(index)] < 0 || order[static_cast<std::size_t>(index)] > 2)
            {
                throw ValueError(std::format("makeRotationByAxes：优先级串第 {} 个字符不是大写 X、Y 或 Z，请改成 \"{}\" 形式", index + 1, "ZXY"));
            }
        }
    ...
    void Rotation::setEulerAngles(EulerSequence order, double alpha, double beta, double gamma)
    {
        ...
        {
            throw ValueError(std::format("setEulerAngles：欧拉序列取值 {} 非法，请传入 Invalid 与 EulerSequenceLast 之间的具体"
        ...
        {
            values[static_cast<std::size_t>(parameters.m_firstAxis)]  = cosSecond * sinCos - sinSecond * cosSin;
            values[static_cast<std::size_t>(parameters.m_secondAxis)] = cosSecond * sinSin + sinSecond * cosCos;
            values[static_cast<std::size_t>(parameters.m_thirdAxis)]  = cosSecond * cosSin - sinSecond * sinCos;
            values[0]                                                 = cosSecond * cosCos + sinSecond * sinSin;
        }
        if (parameters.m_hasOddPermutation)
        {
            // 与前面取反 second 对应，这里把中间分量再翻回来
            values[static_cast<std::size_t>(parameters.m_secondAxis)] = -values[static_cast<std::size_t>(parameters.m_secondAxis)];
        }
    ...
    void Rotation::getEulerAngles(const EulerSequence order, double &alpha, double &beta, double &gamma) const
    {
        ...
        if (order == Invalid || order >= EulerSequenceLast)
        {
            throw ValueError(std::format("getEulerAngles：欧拉序列取值 {} 非法，请传入 Invalid 与 EulerSequenceLast 之间的具体"
                                         "序列，例如 Intrinsic_ZXY；由名字转换时可用 eulerSequenceFromName() 并检查它是否"
                                         "返回 Invalid",
                                         static_cast<int>(order)));
        }
        ...
        if (parameters.m_repeatsFirstAxis)
        {
            ...
            const double sineBeta       = std::sqrt(firstRowSecond * firstRowSecond + firstRowThird * firstRowThird);
            if (sineBeta > 16 * std::numeric_limits<double>::epsilon())
            {
                alpha = std::atan2(elementAt(parameters.m_firstAxis, parameters.m_secondAxis), elementAt(parameters.m_firstAxis, parameters.m_thirdAxis));
                gamma = std::atan2(elementAt(parameters.m_secondAxis, parameters.m_firstAxis), -elementAt(parameters.m_thirdAxis, parameters.m_firstAxis));
            } else
            {
                // 中间角接近 0 或 π：首末角退化，只能定出它们的和差
                alpha = std::atan2(-elementAt(parameters.m_secondAxis, parameters.m_thirdAxis), elementAt(parameters.m_secondAxis, parameters.m_secondAxis));
                gamma = 0.0;
            }
        ...
        } else
        {
            ...
            const double cosineBeta     = std::sqrt(firstRowFirst * firstRowFirst + secondRowFirst * secondRowFirst);
            if (cosineBeta > 16 * std::numeric_limits<double>::epsilon())
            {
                alpha = std::atan2(elementAt(parameters.m_thirdAxis, parameters.m_secondAxis), elementAt(parameters.m_thirdAxis, parameters.m_thirdAxis));
                gamma = std::atan2(elementAt(parameters.m_secondAxis, parameters.m_firstAxis), elementAt(parameters.m_firstAxis, parameters.m_firstAxis));
            } else
            {
