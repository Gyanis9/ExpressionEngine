...
DualQuat::DualQuat(DualQuat realPart, DualQuat dualPart)
    : x(realPart.x.re, dualPart.x.re), y(realPart.y.re, dualPart.y.re),
      z(realPart.z.re, dualPart.z.re), w(realPart.w.re, dualPart.w.re) {
    ...
    if (realPartDualLength >= PureRealTolerance || dualPartDualLength >= PureRealTolerance) {
        throw ValueError(std::format(
            "DualQuat：由实部与对偶部构造时，两个参数都必须是纯实四元数（对偶分量全为零），"
...
double DualQuat::dot(DualQuat left, DualQuat right) {
    // 只比较实部：旋转方向一致与否决定插值是否走短弧
    return left.x.re * right.x.re + left.y.re * right.y.re + left.z.re * right.z.re +
           left.w.re * right.w.re;
}
...
DualQuat DualQuat::pow(double t, bool shorten) const {
    ...
    const DualQuat screwMoment =
        (self.dual().vec() - pitch / 2 * std::cos(theta / 2) * screwAxis) * normalizeMultiplier;
    ...
    return {screwAxis * std::sin(theta / 2) + DualQuat(0.0, 0.0, 0.0, std::cos(theta / 2)),
            screwMoment * std::sin(theta / 2) + pitch / 2 * std::cos(theta / 2) * screwAxis +
                DualQuat(0.0, 0.0, 0.0, -pitch / 2 * std::sin(theta / 2))};
}
}  // namespace ExpressionEngine::Base
