    ...
    for (int row = 0; row < 4; ++row) {
        for (int column = 0; column < 4; ++column) {
            ...
            for (int inner = 0; inner < 4; ++inner) {
                result.m_matrix[row][column] +=
                    m_matrix[row][inner] * other.m_matrix[inner][column];
            }
        }
    }
...
inline void Matrix4D::multVec(const Vector3d& source, Vector3d& destination) const {
    ...
    const double x = (m_matrix[0][0] * source.x + m_matrix[0][1] * source.y +
                      m_matrix[0][2] * source.z + m_matrix[0][3]);
    const double y = (m_matrix[1][0] * source.x + m_matrix[1][1] * source.y +
                      m_matrix[1][2] * source.z + m_matrix[1][3]);
    const double z = (m_matrix[2][0] * source.x + m_matrix[2][1] * source.y +
                      m_matrix[2][2] * source.z + m_matrix[2][3]);
...
inline bool Matrix4D::operator==(const Matrix4D& other) const {
    ...
    for (int row = 0; row < 4; ++row) {
        for (int column = 0; column < 4; ++column) {
            if (std::abs(m_matrix[row][column] - other.m_matrix[row][column]) >
                traits_type::epsilon()) {
                return false;
            }
        }
...
inline std::array<double, 4>& Matrix4D::operator[](unsigned int index) {
    if (index > 3) {
        ...
        throw IndexError(
            "矩阵行下标越界（只允许 0/1/2/3）；请改行下标，或用 getMatrix() 取全部元素");
    }
