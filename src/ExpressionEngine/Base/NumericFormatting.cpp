...
namespace ExpressionEngine::Base
{
    namespace
    {
        ...
        constexpr std::array<BuiltinLocaleEntry, 19> builtinLocaleTable{{
                {.localeKey = "C", .decimalSeparator = ".", .groupingSeparator = "", .primaryGroupingSize = 0, .secondaryGroupingSize = 0},
        ...
        bool isCLocaleIdentifier(const std::string_view identifier)
        {
            static constexpr std::array<std::string_view, 6> cLocaleNames{
                    "c", "posix", "c.utf-8", "c.utf8", "posix.utf-8", "posix.utf8",
            };
        ...
        {
            const auto entry = std::ranges::find(builtinLocaleTable, localeKey, &BuiltinLocaleEntry::localeKey);
         ...
         *          配大量小数位）会需要很长的输出。
         */
        std::string toAsciiNumber(const double value, const std::chars_format notation, const std::optional<int> precision)
        {
            ...
            for (;;)
            {
                ...
                const auto  conversion = precision.has_value() ? std::to_chars(first, last, value, notation, *precision) : std::to_chars(first, last, value, notation);
        ...
        std::string applyGrouping(const std::string_view integerDigits, const NumericLocaleContext &formatting, const bool groupingAllowed)
        {
            ...
            const int   secondary      = formatting.secondaryGroupingSize > 0 ? formatting.secondaryGroupingSize : primary;
            ...
            while (groupEnd > 0)
            {
                ...
                grouped.insert(0, integerDigits.substr(static_cast<std::size_t>(groupBegin), static_cast<std::size_t>(groupEnd - groupBegin)));
                if (groupBegin > 0)
                {
        ...
        std::string localizeDigits(std::string text, const std::string &zeroDigit)
        {
            ...
            if (zeroDigit == "0" || zeroDigit.empty() || !decodeUtf8CodePoint(zeroDigit, zeroBytes, zeroCodePoint) || zeroBytes != zeroDigit.size())
            {
    ...
    std::expected<NumericLocaleContext, NumericLocaleError> createNumericLocaleContext(const std::string_view localeId)
    {
        if (localeId.empty())
        {
    ...
    std::string formatNumericValue(const double value, const int precision, const NumberNotation notation, const bool omitGroupSeparator, const NumericLocaleContext &formatting)
    {
        ...
        switch (notation)
        {
            ...
            default:
                ...
                ascii = precision > 0 ? toAsciiNumber(value, std::chars_format::general, precision) : toAsciiNumber(value, std::chars_format::general, std::nullopt);
                break;
        }
        ...
        std::string exponent;
        if (const auto exponentPosition = body.find_first_of("eE"); exponentPosition != std::string::npos)
        {
        ...
        const std::string_view integerDigits  = hasDecimalPoint ? bodyView.substr(0, pointPosition) : bodyView;
        const std::string_view fractionDigits = hasDecimalPoint ? bodyView.substr(pointPosition + 1) : std::string_view{};
