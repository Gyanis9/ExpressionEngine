...
namespace ExpressionEngine::Base
{
    namespace
    {
        ...
        bool startsAt(const std::string_view text, const std::size_t position, const std::string_view value)
        {
            return !value.empty() && position + value.size() <= text.size() && text.substr(position, value.size()) == value;
        }
        ...
        bool decodeUtf8(const std::string_view text, const std::size_t position, char32_t &codePoint, std::size_t &consumedBytes) noexcept
        {
            ...
            if ((sequenceLength == 3 && value < 0x800) || (sequenceLength == 4 && value < 0x10000) || value > 0x10FFFF || (value >= 0xD800 && value <= 0xDFFF))
            {
        ...
        LocalizedNumberResult invalid(const NumericDiagnosticKind kind, const std::size_t offsetBytes, const std::size_t consumed, const std::size_t lengthBytes = 1)
        {
        ...
        bool decimalAt(const std::string_view input, const std::size_t position, const NumericGrammarPolicy &policy, std::size_t &length)
        {
            length = 0;
            if (startsAt(input, position, policy.decimalSeparator))
            {
                ...
                if (policy.argumentSeparator == ";" && policy.decimalSeparator == "," && position + policy.decimalSeparator.size() < input.size() &&
                    std::isspace(static_cast<unsigned char>(input[position + policy.decimalSeparator.size()])) != 0)
                {
                    return false;
                }
        ...
        bool groupingAt(const std::string_view input, const std::size_t position, const NumericLocaleContext &locale, const NumericGrammarPolicy &policy, const bool alreadyGrouped, std::size_t &length)
        {
            ...
            if (policy.groupingSeparator == " ")
            {
                ...
                int         digit       = 0;
                std::size_t digitLength = 0;
                if (position + length >= input.size() || !localizedDigitAt(input, position + length, locale, digit, digitLength))
                {
                    return false;
                }
            }
        ...
        bool signAt(const std::string_view input, const std::size_t position, const NumericLocaleContext &locale, std::size_t &length, char &canonical)
        {
            length    = 0;
            canonical = 0;
    ...
    NumericGrammarPolicy numericGrammarPolicy(const NumericLocaleContext &locale, const NumericSyntaxContext syntax)
    {
    ...
    bool localizedDigitAt(const std::string_view input, const std::size_t position, const NumericLocaleContext &locale, int &digit, std::size_t &consumedBytes)
    {
        ...
        if (locale.zeroDigit.empty() || !decodeUtf8(locale.zeroDigit, 0, zeroCodePoint, zeroByteCount) || zeroByteCount != locale.zeroDigit.size())
        {
    ...
    LocalizedNumberResult scanLocalizedNumber(const std::string_view input, const NumericLocaleContext &locale, const NumericSyntaxContext syntax)
    {
        ...
        while (position < input.size())
        {
            ...
            if (groupingAt(input, position, locale, policy, grouped, separatorLength))
            {
                ...
                if (position == input.size() || !localizedDigitAt(input, position, locale, nextDigit, nextDigitLength))
                {
                    ...
                    return incomplete(NumericDiagnosticKind::IncompleteGrouping, position, position, canonical, separatorLength);
                }
                continue;
            }
            break;
        ...
        if (totalDigits == 0)
        {
            ...
            if (position == input.size() || !localizedDigitAt(input, position, locale, nextDigit, nextDigitLength))
            {
                return incomplete(NumericDiagnosticKind::IncompleteDecimal, position, position, canonical, decimalLength);
            }
            while (position < input.size())
            {
        ...
        } else
        {
            if (grouped)
            {
                ...
                {
                    return invalid(NumericDiagnosticKind::InvalidGrouping, lastGroupingStart, position, lastGroupingLength);
                }
            }
        ...
        if (position < input.size())
        {
            ...
            if (std::isspace(static_cast<unsigned char>(input[position])) != 0)
            {
                ...
                int         nextDigit       = 0;
                std::size_t nextDigitLength = 0;
                if (next < input.size() && localizedDigitAt(input, next, locale, nextDigit, nextDigitLength))
                {
                    return invalid(NumericDiagnosticKind::UnexpectedSeparator, position, position, next - position);
                }
            }
            ...
            const bool  separatorIsSpaceLike     = decodeUtf8(input, position, separatorCodePoint, separatorBytes) && isSpaceLike(separatorCodePoint);
            const bool  separatorFollowedByDigit = separatorIsSpaceLike && localizedDigitAt(input, position + separatorBytes, locale, nextDigit, nextDigitBytes);
            ...
            std::size_t separatorLength = 0;
            if (decimalAt(input, position, policy, separatorLength) || groupingAt(input, position, locale, policy, grouped, separatorLength))
            {
