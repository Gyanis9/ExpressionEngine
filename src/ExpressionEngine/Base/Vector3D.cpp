...
Vector3<float_type>::Vector3(float_type xValue, float_type yValue, float_type zValue)
    : x(xValue), y(yValue), z(zValue) {
}

template <class float_type> float_type& Vector3<float_type>::operator[](unsigned short index) {
...
template <class float_type> Vector3<float_type> Vector3<float_type>::operator-() const {
...
template <class float_type> Vector3<float_type>& Vector3<float_type>::operator*=(float_type scale) {
    x *= scale;
    y *= scale;
    z *= scale;
    return *this;
...
template <class float_type>
Vector3<float_type> Vector3<float_type>::operator%(const Vector3<float_type>& other) const {
    return Vector3<float_type>((y * other.z) - (z * other.y),
                               (z * other.x) - (x * other.z),
                               (x * other.y) - (y * other.x));
}
...
template <class float_type>
bool Vector3<float_type>::IsOnLineSegment(const Vector3<float_type>& startPoint,
...
bool Vector3<float_type>::operator==(const Vector3<float_type>& other) const {
    ...
    return (std::abs(x - other.x) <= traits_type::epsilon()) &&
           (std::abs(y - other.y) <= traits_type::epsilon()) &&
           (std::abs(z - other.z) <= traits_type::epsilon());
}
...
template <class float_type>
bool Vector3<float_type>::IsParallel(const Vector3<float_type>& direction,
                                     float_type tolerance) const {
...
template <class float_type>
bool Vector3<float_type>::IsNormal(const Vector3<float_type>& direction,
                                   float_type tolerance) const {
    const float_type angle = GetAngle(direction);
    if (std::isnan(angle)) {
        return false;
...
template <class float_type>
Vector3<float_type>& Vector3<float_type>::ProjectToPlane(const Vector3<float_type>& base,
                                                         const Vector3<float_type>& normal) {
...
template <class float_type>
void Vector3<float_type>::ProjectToPlane(const Vector3& base,
...
template <class float_type>
float_type Vector3<float_type>::DistanceToPlane(const Vector3<float_type>& base,
                                                const Vector3<float_type>& normal) const {
    // 除以法向长度，允许传入未归一化的法向
    return ((*this - base) * normal) / normal.Length();
}

template <class float_type> float_type Vector3<float_type>::Length() const {
...
float_type Vector3<float_type>::DistanceToLine(const Vector3<float_type>& base,
                                               const Vector3<float_type>& direction) const {
...
template <class float_type>
Vector3<float_type> Vector3<float_type>::DistanceToLineSegment(const Vector3& firstPoint,
    ...
    const float_type t =
        std::clamp(dot / squaredLength, static_cast<float_type>(0), static_cast<float_type>(1));
    return t * fromFirst - toPoint;
}

template <class float_type>
Vector3<float_type>& Vector3<float_type>::ProjectToLine(const Vector3<float_type>& point,
                                                        const Vector3<float_type>& line) {
    return (*this = ((((point * line) / line.Sqr()) * line) - point));
}

template <class float_type>
Vector3<float_type> Vector3<float_type>::Perpendicular(const Vector3<float_type>& base,
                                                       const Vector3<float_type>& direction) const {
...
template <class float_type> float_type Vector3<float_type>::Sqr() const {
...
template <class float_type> void Vector3<float_type>::ScaleX(float_type factor) {
    x *= factor;
}

template <class float_type> void Vector3<float_type>::ScaleY(float_type factor) {
    y *= factor;
}

template <class float_type> void Vector3<float_type>::ScaleZ(float_type factor) {
    z *= factor;
}
...
template <class float_type> void Vector3<float_type>::MoveX(float_type offset) {
    x += offset;
}

template <class float_type> void Vector3<float_type>::MoveY(float_type offset) {
    y += offset;
}

template <class float_type> void Vector3<float_type>::MoveZ(float_type offset) {
    z += offset;
}
...
template <class float_type> void Vector3<float_type>::RotateX(float_type angle) {
...
template <class float_type> void Vector3<float_type>::RotateY(float_type angle) {
    const Vector3<float_type> original(*this);
...
template <class float_type> void Vector3<float_type>::RotateZ(float_type angle) {
    const Vector3<float_type> original(*this);
...
template <class float_type> Vector3<float_type>& Vector3<float_type>::Normalize() {
...
template <class float_type> Vector3<float_type> Vector3<float_type>::Normalized() const {
    Vector3<float_type> copy = *this;
    copy.Normalize();
    return copy;
}

template <class float_type> bool Vector3<float_type>::IsNull() const {
    const float_type zero{0.0};
    return (x == zero) && (y == zero) && (z == zero);
}

template <class float_type> float_type Vector3<float_type>::GetAngle(const Vector3& other) const {
...
template <class float_type>
float_type Vector3<float_type>::GetAngleOriented(const Vector3& other,
...
template <class float_type>
void Vector3<float_type>::TransformToCoordinateSystem(const Vector3& base,
                                                      const Vector3& dirX,
                                                      const Vector3& dirY) {
