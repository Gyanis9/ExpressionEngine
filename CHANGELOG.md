...
第一个公开发布的版本。0.x 表示接口仍在收敛，条目如下。
破坏性变更只记在这一节里，本节即 v0.0.1 的发布说明。
...
- **宿主自带的单位方案可用**：`UnitsSchemas` 收 `UnitsSchemasDataPack` 构造，不依赖内置表与全局状态；
  ...
  由条目的回调接管整段排版。
- **单位方案能整体装进门面**：新增 `UnitsApi::applyPack()`，宿主用自己的 `UnitsSchemasDataPack`
  替换进程级方案集合后，当前方案、默认小数位数与默认分数分母都取自该数据包并影响所有排版；
  宿主显式设置过的精度与分母跨替换保留。
...
- **体积膨胀系数不再借用线膨胀的类型名**：它此前按 `ThermalExpansionCoefficient` 报告自己，方案
  排版因此取到线膨胀那一档（`mm/mm/K` 与 `m^3/m^3/K` 相差 1e9），换算表里为体积准备的那一行永远
  读不到。同指数的其它别名照旧（抗压强度借 `Pressure`、摩擦角借 `Angle`）——它们自己没有换算条目，
  借名正是为了让排版有落点。
- **单位方案查找的报错更具体**：按名或按编号找不到方案时，文案带上查的是哪个名字、哪个编号
  （此前只有一句「找不到匹配的单位方案」）；换算条目写了未登记的特殊函数名且没给回调时抛
  `Base::ValueError`（此前排出一段空文本，宿主以为方案就这么显示）。
- **文本字符计数与下标定位共用一套切分**：宿主文本里混进孤立的续字节（形如 `10xxxxxx` 却没有归属
  的前导字节）时，`len` 与文本下标用的不再是同一套规则——字符数少算一个，`<<…>>[-1]` 取末字符会
  取到那个孤立字节。现在两者共用同一份实现，切分必然铺满整段文本。
- **矩阵反解旋转轴更稳**：`Matrix4D::toAxisAngle` 在转角贴近 180° 时不再用 `(1+tr)/(2 sin A)`
  求轴上基点（那是 0/0，舍入噪声会把基点甩到轴外，拿反解结果重建就不是原矩阵），改用对角线反解轴、
  基点取轴线垂足；迹因舍入略超 3 时按零转角处理（此前给出 NaN 角度却返回成功）。
